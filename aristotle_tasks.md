...
## Live Status (2026-04-27 10:03 EDT)

- Active jobs (ours): 1/5; canary `09cd85dd` QUEUED ~15h.
  Not waiting per user.
- **Integrated this tick (local Claude-owned, 4 lemmas, 1 new file):**
  - NEW `Jacobian/TraceDegree/PullbackFormsLinearMapIdEvalSmul.lean`:
    bundled along-id form-slot integer-scalar variants + helpers:
      - `pullbackFormsLinearMap_id_nsmul_apply_vec`
      - `pullbackFormsLinearMap_id_zsmul_apply_vec`
      - `pullbackFormsLinearMap_id_neg_zero_apply_vec`
      - `pullbackFormsLinearMap_id_zero_sub_zero_apply_vec`
    Mirrors `PullbackFunIdEvalSmul` at the bundled-LinearMap layer
    via `evalLinearMap_{n,z}smul`. Wired into TraceDegree umbrella.
    With `_id_smul_apply_vec` already in `PullbackFormsLinearMapIdEval`,
    bundled along-id form-slot smul picture is ℂ/ℕ/ℤ-complete.
- **Strategic note (carried forward):**
  Continuing 4-lemma/tick cadence per user directive. Backend
  asleep ~15h — not waiting.

## Earlier (now stale; kept for context only)
## Stale Live Status (2026-04-27 10:00 EDT)
