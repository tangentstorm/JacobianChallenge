...
\uses{thm:period-lattice, prop:complex-torus-package, def:analytic-genus, lem:stable-chart-at-instance}
...
\begin{lemma}[Chart-stability adapter instance (open)]
\label{lem:stable-chart-at-instance}
The \code{StableChartAt} adapter instance threaded through the public manifold /
Lie structure of \(\Jac(X)\). It currently stands as an open obligation
(\lstinline{sorry}) in \code{Jacobian/Solution.lean}; the honest route replaces
it with genuine chart-overlap-derivative continuity (worker jc3).
Tracking: \ghissue{237}.
\lean{JacobianChallenge.Solution.instStableChartAtComplexOfT2SpaceOfCompactSpaceOfConnectedSpaceOfIsManifoldModelWithCornersSelfTopWithTopENat}
\end{lemma}
...
\lean{JacobianChallenge.TraceDegree.analyticDegree, JacobianChallenge.TraceDegree.pullbackFormsLinearMap}
\leanok
\proofplan
