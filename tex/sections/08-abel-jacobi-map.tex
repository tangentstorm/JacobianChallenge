...
anti-hack theorem of the challenge: the geometric
...
it cannot collapse, cheat, or otherwise smudge the
distinction between points. With this section's theorems in hand,
the surface sits inside its Jacobian as a curve.
\end{layman}
...
Restricted to degree-zero divisors, the extension is a group
...
% DEPRECATED (2026-05-31): The 10-round top-down refinement tree for Abel's
% existence theorem (Forster decomposition, Rounds 6-18) rebuilt the general
% Riemann-Roch formula and Serre duality inside the Abel-Jacobi layer. Per the
% live frontier (scripts/list-sorries.py), that tree is off the critical path:
% its Lean decls in Jacobian/AbelJacobi/AnalyticOfCurveBasis.lean carry no
% reachable sorries, and Abel injectivity is now carried by the single named
% obligation `pathIntegralFunctional_separates_points` (consumed by
% `Solution.ofCurve_inj`). The full-Riemann-Roch dependency tree has therefore
% been removed from this blueprint; only the genus-zero-restricted Riemann-Roch
% leaf survives elsewhere (see Section on genus-zero classification).
...
full \(\Div^0(X) \to \Pic^0(X)\) machinery just to apply it once.

\end{layman}

\begin{proof}\leanok
The substantive content is the classical Abel construction: from
third-kind data \(f_0\) with pole divisor \((Q_1)+(Q_2)\), the
Riemann reciprocity (bilinear) identity converts the period-congruence
hypothesis \(-I_{P,Q_1}+I_{P,Q_2}\in\Lambda_{\mathrm{ba}}\) into the
vanishing of the periods of \(d\log f_0\) modulo \(2\pi i\cdot\Z^{2g}\),
and the log-exp construction then produces a meromorphic function
\(f\) with \((f) = (Q_1) - (Q_2)\). In the formal development this
classical input is isolated as the single named obligation
\texttt{pathIntegralFunctional\_separates\_points} (the genus-positive
Abel point-separation statement); the present lemma is its two-point
divisor packaging. Conversion to Theorem~\ref{thm:abel-existence}
(umbrella) is by specialisation to the divisor \(D = (Q_1) - (Q_2)\)
--- the path-integral period congruence is exactly the statement
\(\AJ(D) = 0\) in \(\Jac(X)\).
\end{proof}
...
\(\mathrm{div}_\infty(f) = (Q_2)\). The Lean implementation delegates
to \texttt{assemble\_meromorphicMap}, which builds a fresh
\texttt{MeromorphicMapToSphere} carrier with the prescribed
\texttt{zeroDivisor = (Q\_1)} and \texttt{poleDivisor = (Q\_2)} via a
single-point indicator placeholder at \(Q_2\); the analytic
modulus-divergence axiom delegates in turn to the named frontier
obligation
\texttt{singlePointIndicator\_exists\_modulus\_atTop\_obligation},
which encodes the chart-local \(1/(z - Q_2)\) extension that supplies
the pole behaviour.
\end{proof}

\begin{theorem}[S20: Basis-aligned Riemann-Hurwitz at degree 1]
\label{thm:riemann-hurwitz-deg1-basis-aligned}
\uses{thm:genus-zero-classification}
...
least one pole, leaving no holomorphic 1-forms at all. Why
...
\uses{thm:abel-basis-aligned-two-point, thm:riemann-hurwitz-deg1-basis-aligned}
...
nontrivial Jacobian could distinguish all the points of the
surface.
\end{layman}
...
\begin{lemma}[Abel--Jacobi injectivity for positive genus (open)]
\label{lem:analytic-of-curve-injective}
\uses{thm:abel-point-separation}
For \(g(X)>0\) the Abel--Jacobi map \(\AJ_P\) is injective. Follows from the
point-separation theorem; matches the frozen public \code{ofCurve\_inj}
signature. Open obligation (\code{sorry}) until the genus-zero single-pole
route lands. Consumed by the challenge API (\ref{thm:challenge-api}).
Tracking: \ghissue{224}.
\lean{JacobianChallenge.AbelJacobi.analyticOfCurve_injective}
\end{lemma}

\begin{lemma}[Universe-polymorphic point separation (open)]
\label{lem:path-integral-functional-separates-points-u}
\uses{thm:abel-point-separation}
Universe-\(u\) companion of the point-separation theorem, needed by the public
\code{ofCurve\_inj} for \(X:\mathrm{Type}\,u\). Open obligation
(\code{sorry}). Consumed by the challenge API (\ref{thm:challenge-api}).
Tracking: \ghissue{225}.
\lean{JacobianChallenge.AbelJacobi.pathIntegralFunctional_separates_pointsU}
\end{lemma}
