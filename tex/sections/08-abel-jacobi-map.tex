...
\uses{thm:abel-existence,def:abel-jacobi}
...
\begin{proof}\leanok
Specialise Theorem~\ref{thm:abel-existence} to the divisor
\(D = (Q_1) - (Q_2)\). Path-integral period congruence at two
distinct points is exactly the statement \(\AJ(D) = 0\) in
\(\Jac(X)\), so Abel's existence direction supplies the required
\(f\) with \((f) = D\).
\end{proof}
