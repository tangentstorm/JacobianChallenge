...
The document opens with two short cataloging parts. Part~1, the
\emph{Statement of the Jacobian Challenge}, lists the public-API
obligations (genus, Jacobian, Abel--Jacobi map, degree, pushforward,
pullback, plus the four anti-hack theorems) one for one with
\code{Jacobian/Challenge.lean}. Part~2, the \emph{Statement Bank},
collects the internal helper statements (period pairing, period
...
through the Montel--Riesz route. Section~3 packages
Riemann--Roch and the sheaf-cohomology infrastructure feeding it.
Section~4 develops the branched-cover degree theory and reaches
the genus-zero classification — the first anti-hack theorem.
Section~5 develops the \emph{polygonal model} — the classical
unfolding of the surface onto a flat \(4g\)-gon — that underlies
all later integration arguments. Section~6 builds the period
pairing and the Riemann bilinear relations. Section~7 constructs
the Jacobian via the complex-torus package. Section~8 establishes
the Abel--Jacobi map and its injectivity. Section~9 develops
degree, trace, pullback and pushforward, and the trace identity.
Section~10 assembles the public API. Section~11 maps each Lean
...
Part~1 below restates that public target informally; Part~2 collects the
internal helper statements; the remaining sections build up the supporting
mathematics.
