...
underlying surface is the genus-\(g\) closed orientable
surface and not some exotic close cousin.
\end{layman}

\begin{lemma}[Standard word quotient is the standard polygon]
\label{lem:polygon4g-eq-standard-word-quotient}
For the standard edge word, the word quotient is the same type as the
standard \(4g\)-gon quotient.
...
Without Rad\'o's theorem the entire Stage-A machinery has no input.
...
b_2^{-1}\); and so on. Why bother? Combined with Rad\'o's theorem
...
\begin{proofsketch}
Triangulate \(X\) (Rad\'o's theorem: every compact surface admits a
...
spinning.'' The M\"obius strip and Klein bottle are not orientable;
...
\begin{layman}
The Hodge decomposition theorem says that on a compact K\"ahler
...
\uses{def:topological-genus, input:hodge-deRham, lem:cut-along-non-tree-yields-unfolded-disk, lem:polygon4g-singular-simplex-subdivision-lifts-to-disk, lem:polygon4g-partial-side-arc-homologous-to-edge-chain, lem:edge-chain-sum-singular-boundary-scalar-coefficient-zero, lem:handleprefix-tailrotate-homeomorph}
...
% --- Open surface-classification obligations feeding the polygonal model ---
% These are the reachable `sorry`s underneath the analytic=topological genus
% identification; each is a tracked open obligation in the Lean development.

\begin{lemma}[Cut along non-tree edges yields an unfolded disk (open)]
\label{lem:cut-along-non-tree-yields-unfolded-disk}
Cutting a triangulated compact surface along the non-tree edges of a dual
spanning tree produces a disk (the fundamental polygon). Open obligation
(\code{sorry}) in the Lean development.
Tracking: \ghissue{226}.
\lean{JacobianChallenge.Periods.cut_along_nonTree_yields_unfoldedDisk}
\end{lemma}

\begin{lemma}[Polygon quotient local-lift substrate]
\label{lem:polygon4g-quotient-local-lift-substrate}
\uses{lem:polygon4g-eq-standard-word-quotient}
The quotient-path lift frontier is isolated above a sorry-free local
substrate: the finite-subdivision data structure records the required lift
pieces and adjacent endpoint relations, while interior quotient fibres are
already singleton fibres of the disk quotient map.
\lean{JacobianChallenge.Periods.Polygon4gQuotientPathFiniteLiftSubdivision,
JacobianChallenge.Periods.polygon4g_sideRel_eq_of_norm_lt_one,
JacobianChallenge.Periods.polygon4g_mk_eq_of_norm_lt_one}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Polygon quotient path admits a finite chart-lift subdivision (open)]
\label{lem:polygon4g-quotient-path-finite-lift-subdivision}
\uses{lem:polygon4g-quotient-local-lift-substrate}
A path in the \(4g\)-gon side-pairing quotient subdivides into finitely many
chart-liftable pieces. Open obligation (\code{sorry}).
Tracking: \ghissue{228}.
\lean{JacobianChallenge.Periods.polygon4g_quotient_path_finite_lift_subdivision}
\end{lemma}

\begin{lemma}[Polygon quotient simplex lift package (open)]
\label{lem:polygon4g-singular-simplex-subdivision-lifts-to-disk}
\uses{lem:polygon4g-quotient-path-finite-lift-subdivision}
A singular one-simplex in the polygon quotient admits the full disk-lift
package: finitely many lifted pieces, adjacent endpoint relations, and the
singular two-chain witnessing that the subdivided projected chain is homologous
to the original simplex. Besides the quotient-chart finite-lift topology, this
package also depends on the singular-chain subdivision-prism frontier in
`Hurewicz.lean`; that frontier remains open and is not marked green here.
\lean{JacobianChallenge.Periods.polygon4g_singularSimplex_subdivision_lifts_to_disk}
\end{lemma}

\begin{lemma}[Endpoint repair and side-arc bookkeeping substrate]
\label{lem:polygon4g-endpoint-repair-bookkeeping-substrate}
\uses{lem:polygon4g-quotient-local-lift-substrate}
Endpoint repair for identified boundary points is expressed by a concrete
disk-side chain, its quotient pushforward, and the edge-basis coefficient
bookkeeping. The reflexive repair, quotient pushforward identities, and
canonical \(a_i\)/\(b_i\) edge-index calculations are already formalized
without the primitive side-strip geometry.
\lean{JacobianChallenge.Periods.Polygon4gEndpointRepairData,
JacobianChallenge.Periods.polygon4gBoundaryArcStep_projected_eq_chainMap,
JacobianChallenge.Periods.polygon4gBoundaryArcSteps_projected_eq_chainMap,
JacobianChallenge.Periods.polygon4gEndpointRepair_projected_eq_chainMap,
JacobianChallenge.Periods.polygon4g_endpoint_pair_repaired_refl,
JacobianChallenge.Periods.polygon4g_aPair_edgeIndex,
JacobianChallenge.Periods.polygon4g_bPair_edgeIndex,
JacobianChallenge.Periods.edgeArcIdx_aPair_edgeIndex,
JacobianChallenge.Periods.edgeArcIdx_bPair_edgeIndex}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Partial side arc homologous to an edge chain (open)]
\label{lem:polygon4g-partial-side-arc-homologous-to-edge-chain}
\uses{lem:polygon4g-endpoint-repair-bookkeeping-substrate}
A partial boundary side-arc of the \(4g\)-gon is homologous to the corresponding
edge chain. Open obligation (\code{sorry}).
Tracking: \ghissue{229}.
\lean{JacobianChallenge.Periods.polygon4g_partial_side_arc_homologous_to_edge_chain}
\end{lemma}

\begin{lemma}[Edge-chain finite coefficient substrate]
\label{lem:edge-chain-finite-coefficient-substrate}
The scalar edge-boundary frontier is reduced to a finite coefficient
calculation once a singular two-boundary is expanded into signed
two-simplex-face terms. The project already proves the comparison from the
signed target-edge coefficient through the flattened face-sum and
edge-simplex forms.
\lean{JacobianChallenge.Periods.EdgeBoundarySignedFaceCoefficientComparison,
JacobianChallenge.Periods.signedFaceTargetEdgeCoefficient_eq_of_edgeSimplex_sum,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_signed_face_terms_edgeSimplex_coefficient_comparison,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_signed_face_terms_edgeSimplex_scalar_coefficient_zero,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_face_terms_edgeSimplex_scalar_coefficient_zero,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_faces_edgeSimplex_scalar_coefficient_zero,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_faces_scalar_coefficient_zero,
JacobianChallenge.Periods.edgeChain_sum_singular_boundary_decomposition_scalar_coefficient_zero}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Edge-chain boundary coefficients vanish (open)]
\label{lem:edge-chain-sum-singular-boundary-scalar-coefficient-zero}
\uses{lem:edge-chain-finite-coefficient-substrate}
If a singular two-boundary is a finite sum of edge chains, every edge
coefficient is zero (edge independence in \(H_1\)). Open obligation
(\code{sorry}).
Tracking: \ghissue{230}.
\lean{JacobianChallenge.Periods.edgeChain_sum_singular_boundary_scalar_coefficient_zero}
\end{lemma}

\begin{lemma}[Handle-prefix tail-rotation homeomorphism (open)]
\label{lem:handleprefix-tailrotate-homeomorph}
Rotating the tail of a handle-prefixed edge word gives a homeomorphic quotient
(a Tietze handle-swap step in the surface classification). Open obligation
(\code{sorry}).
Tracking: \ghissue{239}.
\lean{JacobianChallenge.Periods.handlePrefix_tailRotate_homeomorph_nonzero}
\end{lemma}
