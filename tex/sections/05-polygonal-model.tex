...
\uses{def:polygon4g,lem:polygon4g-eq-standard-word-quotient,def:tietze-equivalence,def:orientable,input:rado-triangulation,input:tietze-normal-form}
...
\uses{def:topological-genus,input:de-rham-theorem,input:hodge-decomposition,input:dolbeault}
...
\code{Jacobian/Blueprint/Sec03/PrimitiveOnPolygon.lean}, three of which
are sorry-free real-claim forwarders to the substantive lemmas in
\code{Jacobian/Periods/PrimitiveOnPolygon.lean}:
\code{primitive\_on\_polygon\_mk\_injOn} (sub-leaf 1, injectivity on
\(\mathrm{OpenDisk}\)), \code{primitive\_on\_polygon\_image\_isOpen}
(sub-leaf 2, openness of the image), and
\code{primitive\_on\_polygon\_disk\_primitive} (sub-leaf 3, holomorphic
primitive on the convex disk via Mathlib's Cauchy theorem). The
fourth, \code{primitive\_on\_polygon\_chart\_pullback}, is the frontier
obligation: pulling \(\omega\) back along the open-disk chart to a
\code{DifferentiableOn} function, blocked on the complex-manifold
structure on \(\mathrm{Polygon4g}(g)\). The headline
\code{primitive\_on\_polygon} is a sorry-free assembly of the four.
\end{proofsketch}

\begin{formalizationnote}
The current formalization splits this lemma into three Aristotle-sized
sub-leaves, all sorry-free
(\ref{lem:mk-injOn-openDisk}, \ref{lem:mk-image-openDisk-isOpen},
\ref{lem:holomorphic-primitive-openDisk}). The umbrella statement
above remains \emph{not ready} pending a complex-manifold structure on
\(\mathrm{Polygon4g}\), which would let the holomorphic 1-form
\(\omega\) be pulled back along the chart
\(\mathrm{mk}_g|_{\mathrm{open\,disk}}\) and the resulting scalar
function fed to
\ref{lem:holomorphic-primitive-openDisk}.
