...
\[
  f^{*}:\HY\to\HX.
\]
\lean{JacobianChallenge.TraceDegree.pullbackFormsLinearMap,periodPairing_pullbackFormsBundledLM}
\leanok
\end{lemma}
...
multiples come along for the ride) and lands in holomorphic
...
\[
  \tr_f:\HX\to\HY.
\]
\lean{JacobianChallenge.TraceDegree.analyticPushforward,JacobianChallenge.HolomorphicForms.trace_pullback_at_regular_value,JacobianChallenge.HolomorphicForms.traceFormsBundled_eq_zero_of_constant}
\leanok
\end{lemma}
...
\[
  ...
  d\,\eta.
\]
\lean{JacobianChallenge.TraceDegree.traceFormsBundledLM_pullbackFormsBundledLM_eq_degree_smul}
Both sides are holomorphic one-forms on \(Y\), so equality on the complement
of the finite branch locus extends across the branch locus via the identity principle.
\lean{JacobianChallenge.HolomorphicForms.holomorphicOneForm_ext_on}
\end{proof}
...
\]
\lean{JacobianChallenge.Solution.Jacobian₀.pullback, JacobianChallenge.Solution.Jacobian₀.pushforward}
\leanok
\end{lemma}
...
each side. Once that's true, both maps descend to homomorphisms
