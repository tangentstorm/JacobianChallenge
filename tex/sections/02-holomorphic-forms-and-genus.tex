...
The first deep fact, and the engine of this section, is that \(g(X)\)
is finite. The argument is a triumph of twentieth-century complex
analysis: equip \(H^0(X,\Omega^1_X)\) with a sup-norm; show that the
unit ball is compact (\emph{Montel's theorem}: a bounded sequence of
holomorphic functions has a uniformly convergent subsequence); apply
Riesz's lemma (a normed space whose unit ball is compact must be
finite-dimensional). The compactness of the surface is what makes the
sup-norm finite in the first place — on the entire complex plane,
the dimension of holomorphic 1-forms is uncountably infinite.

From finite-dimensionality, the rest of the section unspools. The
\emph{Riemann--Roch theorem} relates the dimension of the space of
meromorphic functions with prescribed pole bounds to the genus and
the divisor's degree. It is the linchpin of the entire classical
theory: from it we deduce the existence of degree-one meromorphic
functions on any genus-zero surface, hence the genus-zero
classification (the first ``anti-hack'' theorem of the challenge:
analytic genus zero is equivalent to being topologically a sphere).
A subsidiary thread develops the \emph{branched degree} of a
holomorphic map and tools to compute its fibres — the machinery
needed for the Riemann--Hurwitz formula and the genus-zero
classification's homeomorphism step.

Why bother? Genus is the single most important invariant of a
Riemann surface; it controls everything else. Tying it down rigorously
to a finite-dimensional vector space dimension is what lets all the
later constructions — Jacobian, Abel--Jacobi, push-pull — even be
\emph{stated}.
\end{layman}
...
\uses{input:finite-dimensionality,def:analytic-genus}
...
\uses{def:vanishing-order,thm:branched-degree-well-defined}
...
"closed-discrete subset of compact ⇒ finite".}
...
\(f^{-1}(y)\subseteq U\).
\depends{\(U^c\) is closed-in-compact ⇒ compact; image \(f(U^c)\) is
compact ⇒ closed in \(T_2\); \(y_0\notin f(U^c)\); take
\(V:=f(U^c)^c\).}
...
Mathlib (preconnected target ⇒ globally constant).}
