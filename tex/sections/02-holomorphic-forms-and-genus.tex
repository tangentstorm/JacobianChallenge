...
\depends{Mathlib's analytic inverse function theorem
(\texttt{AnalyticAt.localInverse}) plus chart transport.}
...
\depends{Mathlib's \texttt{AnalyticAt.analyticOrderAt\_eq\_natCast}
gives the local form \(f(t)=t^k\cdot g(t)\) with \(g(0)\ne 0\); a
holomorphic \(k\)-th root of \(g\) flattens this to \(s\mapsto s^k\),
...
\uses{lem:branch-locus-finite, lem:local-inj-unramified, lem:local-kfold-ramified}
...
\depends{Combine the local-injectivity lemma at unramified preimages
(contribution \(1\) each) with the local \(k\)-fold structure at
ramified preimages (\(k\) simple preimages contributing \(1\) each, or
the single ramified preimage of weight \(k\), depending on the target
point); the disjoint-union choice uses \texttt{T2Space} on \(X\) and
finiteness of the fibre.}
