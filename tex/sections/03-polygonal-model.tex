...
classification of section 2 would be circular.
...
through PL or smooth-triangulation methods.}
...
\uses{def:polygon4g,lem:polygon4g-eq-standard-word-quotient,def:tietze-equivalence,def:orientable,input:rado-triangulation}
...
project in its own right, currently in progress (edge-word algebra
and Tietze relation are sorry-free; the umbrella reduction theorem
remains).}
...
Hodge decomposition for Riemann surfaces. The project's Lean target
is currently a sorry-bound assembly through which the
...
\(g_{\mathrm{top}}(X) = g_{\mathrm{an}}(X)\).}
