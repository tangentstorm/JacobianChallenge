...
\uses{def:polygon4g,lem:polygon4g-eq-standard-word-quotient}
...
\uses{thm:polygonal-model,def:topological-genus}
For a compact connected oriented Riemann surface \(X\),
\[
  g_{\mathrm{an}}(X) = g_{\mathrm{top}}(X).
\]
\depends{Combines the polygonal-model homeomorphism with a
homology computation on \(\mathrm{Polygon4g}(g)\) (rank \(2g\)
from the \(2g\) symplectic loops). The homology computation is
itself nontrivial; tracked under Stage B.}
