...
\uses{def:branched-degree,def:divisors-on-X}
A meromorphic map \(f:X\to\CPone\) with \(\deg(\poles(f))=1\) admits a
\code{BranchedCoverData} of branched degree 1.
\lean{JacobianChallenge.HolomorphicForms.MeromorphicMapToSphere.exists\_branchedCoverData\_of\_pole\_degree\_one}
\end{lemma}
\begin{proof}
Take the project's analytic constructor (the still-open Sec02 leaf 8
\code{branchedCoverData\_of\_nonconstant\_holomorphic}) applied to the
nonconstant holomorphic map \(f:X\to\CPone\); the branched degree
matches the pole degree.
\end{proof}
