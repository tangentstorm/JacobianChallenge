...
\begin{lemma}[Montel overlap agreement kernel (open)]
...
\begin{lemma}[Montel overlap agreement cross-chart case (open)]
...
\begin{lemma}[Montel chart transition reduction (open)]
...
\uses{lem:montel-two-chart-source-cover}
The raw finite Montel data chooses the normalized identity and inversion local
Montel chart patches, together with local realization witnesses showing that
the returned global patch coordinates are the corresponding chart-ball limits.
This is the finite-cover frontier before forgetting local realizations to raw
chart balls and source-coordinate maps.
...
\uses{lem:montel-normalized-limit-two-chart-cover-with-local-realizations, lem:montel-coherent-local-selector}
The coherent local-realization-backed finite-cover data constructs the
candidate uniformization existentially, then proves that the selected local
patch coordinates represent it in each public target chart and that the inverse
branches represent the candidate inverse. This is the gluing/coherence frontier
separated from the finite-cover selection step.
...
\uses{def:rr-germ-space-carrier, def:rr-germ-space-dimension-input}
...
for the Riemann--Roch difference term; it does \emph{not} constitute a completed
RR vector-space API or the actual dimension theorem. This is the single hard
core that must be built from the project's divisor, meromorphic-function, and
\(\code{analyticGenus}\) / holomorphic-one-form finiteness machinery.
...
\uses{lem:rr-dim-L-point-eq-two-core}
...
\uses{lem:rr-dim-L-point-eq-two-core, lem:rr-K-minus-point-vanishes, lem:rr-germ-space-outside-constants}
...
\end{gathered}
\]
At present that declaration projects from the fixed-pole provider instead of
being a linear-algebra consequence of a genuine RR space. It is therefore part
of the decomposition target, not evidence that the core has been proved.
...
\uses{def:branched-degree}
\lean{JacobianChallenge.HolomorphicForms.meromorphicMapToSphere_zeroDivisor_degree_eq_branchedDegree}
The degree of the zero divisor of a meromorphic map equals the branched degree of the map.
\end{lemma}
...
\lean{JacobianChallenge.HolomorphicForms.genusZero_riemannRoch_K_minus_point_dim_zero}
The cited Lean declaration is currently a structural placeholder (\(\exists \ell, \ell = 0\)). The real statement awaits the \(K-[P]\) germ-space carrier (see jc0's \code{GenusZeroPointRiemannRochGermSpaceDimensionInput} work). 

Once formalized, the intended statement is: The Riemann-Roch space \(L(K - [P])\) is trivial in genus zero.
\end{theorem}
\begin{proof}
(Not yet formalized). Since the canonical degree is -2, the degree of \(K-[P]\) is -3. Any non-zero section would have a principal divisor of degree 0, but \(0 \le \deg(D + (f)) = -3 + 0\), a contradiction.
\end{proof}
