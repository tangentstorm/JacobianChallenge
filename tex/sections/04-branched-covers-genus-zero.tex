...
\begin{lemma}[Green's candidate: cap, modification, profile, harmonicity, germ (open)]
\label{lem:perron-greens-candidate-pole-profile}
\uses{def:perron-greens-family, lem:perron-greens-family-basics, lem:perron-sub-on-poisson-modify, lem:m4-poisson-solution, lem:harmonic-disc-comparison, lem:perron-envelope-center-touch, lem:w1-weak-max-principle}
The W9 frontier statements (slices G2–G4).  \emph{Uniform upper cap}
(G2): every member satisfies
\(v \le \texttt{truncatedLogCap}\,p\,\rho + C\) on \(V \setminus \{p\}\)
with one member-independent constant — §3.3's two-zone bound in one
formula; with \(V \subseteq B(p, R_0)\), the punctured-disc comparison
against \(\varepsilon + (C_v - \log\delta)\,(\log R_0 - \log\lVert\cdot-p\rVert)/\log(R_0/\delta)\)
washes out the member-dependent cap as \(\delta \to 0\), leaving
\(C = \log(R_0/\rho)\); pointwise boundedness of the family follows
(\texttt{greensFamily\_bddAbove}, already assembled).  \emph{Poisson
...
modification disc).  \emph{Two-sided pole profile} (G2):
\(|\texttt{greensCandidate}\,V\,p + \log\lVert\cdot - p\rVert| \le C\)
eventually at \(p\) — lower half landed in the basics, upper half from
the cap through \(\texttt{perronEnvelope\_le}\).  \emph{Off-pole
harmonicity} (G3): conditional on the W7-ii obligation
\(\texttt{PerronEnvelopeHarmonicInput}\) (W7-i's exact hypothesis list,
conclusion upgraded to harmonic-throughout; jc11's lane discharges it
by thin adapter), instantiated on comparison discs inside the punctured
domain.  \emph{W6 germ} (G4): the shifted candidate is harmonic and
bounded on a punctured disc, so the removable-singularity surface
...
\lean{JacobianChallenge.HolomorphicForms.greensFamily_le_truncatedLogCap_add, JacobianChallenge.HolomorphicForms.greensFamily_bddAbove, JacobianChallenge.HolomorphicForms.greensFamily_poissonModificationInput, JacobianChallenge.HolomorphicForms.greensCandidate_pole_profile, JacobianChallenge.HolomorphicForms.PerronEnvelopeHarmonicInput, JacobianChallenge.HolomorphicForms.greensCandidate_harmonicOnNhd_off_pole, JacobianChallenge.HolomorphicForms.greensCandidate_add_log_tendsto}
\end{lemma}
