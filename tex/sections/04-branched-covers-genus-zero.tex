...
\end{lemma}
\begin{proof}
The chart transition between the identity chart and the inversion
chart on \(\OnePoint\,\CC\) is \(z\leftrightarrow w=z^{-1}\). The
chain rule for cotangent vectors then gives, for a 1-form
\(\omega=f(z)\,dz=g(w)\,dw\),
\[
  f(z)\,dz = f(w^{-1})\,d(w^{-1}) = -f(w^{-1})\,w^{-2}\,dw.
\]
Equating with \(g(w)\,dw\) and rearranging gives the displayed
formula on the overlap \(w\neq 0\).
\end{proof}
