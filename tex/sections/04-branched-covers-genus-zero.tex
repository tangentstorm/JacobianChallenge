...
\lean{genusZeroMontel_normalized_limits_agree_on_overlaps}
\end{lemma}
...
\lean{eqOn_of_tendstoLocallyUniformlyOn_same}
\leanok
\end{lemma}
...
\uses{lem:montel-normalized-limit-two-chart-cover-without-coord-rep, lem:montel-coherent-local-selector}
The normalized finite-cover data must represent the candidate homeomorphism in
each public target chart, and its inverse branches must represent the candidate
...
\uses{lem:montel-normalized-limit-two-chart-cover-without-coord-rep, lem:montel-finite-cover-coordinate-representation}
Combining the raw finite-cover data with coordinate representation gives the
normalized identity/inversion chart-ball cover. This provider no longer carries
a separate source-cover obligation; the cover is recovered from the returned
family's \code{patch_cover} field.
...
\lean{JacobianChallenge.HolomorphicForms.genusZeroMontel_raw_global_patch_family_contMDiff}
\end{lemma}
