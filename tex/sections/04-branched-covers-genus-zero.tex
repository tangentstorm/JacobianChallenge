...
\uses{lem:montel-local-patch-realization}
...
\begin{lemma}[Montel two-chart source cover (open)]
...
manifold \(X\). This is the real
\code{source\_cover\_two\_chart} field of
\code{GenusZeroTwoChartMontelCoverSelector}. The missing provider is the
compactness/selection step that proves the two chosen chart-ball domains cover
\(X\): each point has a normalized Montel chart-ball neighborhood, so
...
\lean{JacobianChallenge.HolomorphicForms.GenusZeroTwoChartMontelCoverSelector.source_cover_two_chart}
\end{lemma}
...
\uses{lem:montel-two-chart-source-cover, lem:montel-two-chart-target-cover}
The two chosen public-chart patches must retain their normalized chart-ball
limit packages, one for the identity chart and one for the inversion chart,
so the finite cover remembers the local Montel normalization used before
global gluing.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedLimitTwoChartMontelCoverSelector.identity_normalizedLimit, JacobianChallenge.HolomorphicForms.GenusZeroNormalizedLimitTwoChartMontelCoverSelector.inversion_normalizedLimit}
\end{lemma}
...
The normalized two-chart cover must finally be forgotten down to the finite
\code{GenusZeroGlobalPatchFamily}: the finite index type, patch family,
source-cover field, and target-chart-cover field. The structure and its cover
fields are real, and the two-chart selector now supplies both the source-cover
and target-chart-cover packaging witnesses. The remaining missing provider is
the construction that packages the two-chart normalized-limit selector as this
global family.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily, JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily.patch_cover, JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily.target_chart_cover, JacobianChallenge.HolomorphicForms.GenusZeroTwoChartMontelCoverSelector.patch_cover_global, JacobianChallenge.HolomorphicForms.GenusZeroTwoChartMontelCoverSelector.target_chart_cover_global}
\end{lemma}

\begin{lemma}[Montel selector finite patch-family assembly (open)]
\label{lem:montel-selector-finite-patch-family-assembly}
\uses{lem:uniformization-genus-zero-homeomorphism, lem:montel-global-patch-family-packaging}
The remaining selector-family provider is the global assembly step: choose
finitely many normalized Montel-limit chart balls, attach the public
\(\code{OnePoint}\,\C\) target charts, prove their source cover and target
cover, and package the result as a \code{GenusZeroGlobalPatchFamily}. The
subnodes above expose the local-patch, coherence, two-chart-cover, normalized
limit, and final packaging obligations. No green Lean declaration currently
packages this finite global extraction.
\end{lemma}

\begin{lemma}[Montel selector patch family extraction (open)]
\label{lem:montel-selector-patch-family-extraction}
\uses{lem:montel-selector-finite-patch-family-assembly}
The first field of a normalized Montel patch selector is the extracted finite
\code{GenusZeroGlobalPatchFamily}: normalized source patches, their public
\(\code{OnePoint}\,\C\) target charts, a source cover, and a target-chart
cover. The field projection is real; the open work is the assembly provider
above.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedMontelPatchSelector.family}
\end{lemma}

\begin{lemma}[Montel selector coordinate representation (open)]
\label{lem:montel-selector-coord-represents-uniformization}
\uses{lem:montel-selector-patch-family-extraction}
Given the extracted patch family, each selected local coordinate must represent
the chosen global uniformization in its assigned target chart:
\code{targetChart.symm (coord x) = uniformization x}. This is the
\code{coord\_represents\_uniformization} field of
\code{GenusZeroNormalizedMontelPatchSelector}; it is a separate open provider
leaf until the Montel-limit coordinates are tied to the global map.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedMontelPatchSelector.coord_represents_uniformization}
\end{lemma}

\begin{lemma}[Montel selector inverse-coordinate representation (open)]
\label{lem:montel-selector-invcoord-represents-uniformization}
\uses{lem:montel-selector-patch-family-extraction}
Given the extracted patch family, each selected inverse coordinate branch must
represent the inverse of the global uniformization on the assigned target
chart:
\code{invCoord z = uniformization.symm (targetChart.symm z)}. This is the
\code{invCoord\_represents\_uniformization} field of the selector and remains a
separate open provider leaf.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedMontelPatchSelector.invCoord_represents_uniformization}
\end{lemma}

\begin{lemma}[Montel selector forward smoothness (open)]
\label{lem:montel-selector-uniformization-contmdiff}
\uses{lem:montel-selector-patch-family-extraction, lem:montel-selector-coord-represents-uniformization}
The global uniformization represented by the selected local coordinates must be
\(\code{ContMDiff}\). This is the
\code{uniformization\_contMDiff} field of
\code{GenusZeroNormalizedMontelPatchSelector}; later refinements should connect
it to the normalized local-chart and two-chart \(\code{OnePoint}\,\C\) atlas
substrate.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedMontelPatchSelector.uniformization_contMDiff}
\end{lemma}

\begin{lemma}[Montel selector inverse smoothness (open)]
\label{lem:montel-selector-inverse-uniformization-contmdiff}
\uses{lem:montel-selector-patch-family-extraction, lem:montel-selector-invcoord-represents-uniformization}
The inverse of the global uniformization represented by the selected inverse
coordinate branches must be \(\code{ContMDiff}\). This is the
\code{inverse\_uniformization\_contMDiff} field of
\code{GenusZeroNormalizedMontelPatchSelector}; later refinements should connect
it to inverse-branch regularity in the normalized Montel substrate.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroNormalizedMontelPatchSelector.inverse_uniformization_contMDiff}
\end{lemma}
...
\uses{lem:uniformization-genus-zero-homeomorphism, lem:montel-selector-patch-family-extraction, lem:montel-selector-coord-represents-uniformization, lem:montel-selector-invcoord-represents-uniformization, lem:montel-selector-uniformization-contmdiff, lem:montel-selector-inverse-uniformization-contmdiff}
From a topological homeomorphism \(X\simeq\code{OnePoint}\,\C\), construct the
\code{GenusZeroNormalizedMontelPatchSelector}. This node packages the five
graph-visible selector obligations above into the Lean provider
