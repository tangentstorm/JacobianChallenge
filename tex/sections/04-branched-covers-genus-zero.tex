...
\input{04-montel-coord-represents}
...
\begin{lemma}[Raw-source-chart realized-patch family]
\label{lem:montel-raw-source-chart-realized-family}
\uses{lem:montel-raw-source-chart-package}
The indexed family form of the raw-chart realized-patch corollary, sharpening the
engine's consumption boundary. The engine's final-edge payload does not consume
one realized patch but a \emph{materialized} indexed family
\(\code{realizedPatch}:\forall i,\ \code{MontelRealizedPatch}\,X\,(\code{localPatch}\,i)\)
over its patch index, whereas
\code{exists\_montelRealizedPatch\_of\_rawChart} yields only the single-patch
\code{Nonempty}. From a family of raw atlas charts \(\varphi\,i\), each with the
M-D hypotheses for \(\code{localPatch}\,i\) (per-index smoothness and
inverse-branch target membership), this produces the materialized pi-type
\(\code{Nonempty}\,(\forall i,\ \code{MontelRealizedPatch}\,X\,(\code{localPatch}\,i))\).
Since \code{MontelRealizedPatch} is \emph{data}, the family is materialized by
\code{Classical.choice} through each pointwise \code{Nonempty}; no \code{Fintype}
or \code{Nonempty}\,\(\iota\) is consumed, so the statement is at full generality
\(\iota:\code{Type}^*\) and the engine instantiates \(\iota\) at its
\code{Fintype}-indexed \code{PatchIndex}. Assembly-only over the M-D corollary —
no new analytic content.
\lean{JacobianChallenge.HolomorphicForms.exists_montelRealizedPatch_family_of_rawChart}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Realized-patch finite-cover packaging adapter]
\label{lem:montel-realized-patch-cover-packaging}
\uses{lem:montel-raw-source-chart-realized-family}
The green assembly seam from the realized-patch toolkit into the Perron engine's
selection target. The engine's selection node
\code{genusZeroMontel\_finite\_normalized\_chartBall\_cover\_with\_realized\_patches}
owes three things: choose the normalized chart-ball limits (engine spine), build
the realized Montel patch bundles for those coordinates (the M-D/M-E corollaries),
and package the finite-cover existential. This adapter handles the packaging:
given an \emph{explicit} realized patch family
\(\code{realizedPatch}:\forall i,\ \code{MontelRealizedPatch}\,X\,(\code{localPatch}\,i)\)
over a finite nonempty index, together with the bookkeeping facts stated about it
(source cover, target cover, \code{targetChart} agreement, and the two-chart
clause: identity/inversion indices, exhaustive alternative, and the two
\code{NormalizedChartBallLimit} nonemptiness facts), it produces the existential
conclusion of the selection target verbatim. Taking the realized family
explicitly lets the engine control the concrete witness it proves the bookkeeping
about, rather than reconstructing it from a \code{Nonempty.some}. Pure existential
repackaging --- no analytic content, and not gated on the open analytic spine
(it consumes the spine's outputs as hypotheses). This is the callee
\code{lem:stage-engine-provider-final-edge} discharges the selection packaging
with.
\lean{JacobianChallenge.HolomorphicForms.exists_realizedPatchCover_of_components}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Two-chart cover packaging]
\label{lem:montel-realized-patch-cover-twochart}
\uses{lem:montel-realized-patch-cover-packaging}
The \code{Bool}-indexed specialization of the cover-packaging adapter to the
genus-zero two-chart structure. The genus-zero finite cover is always exactly two
charts --- one realizing the identity target chart, one the inversion chart --- and
the selection conclusion carries the exhaustive alternative
\(\forall i,\ i=\code{identityIndex}\lor i=\code{inversionIndex}\). The general
packaging adapter is stated over an arbitrary \code{Fintype}, so a two-patch
caller must still build a \code{Fintype} family and discharge the exhaustive
alternative by hand. \code{exists\_realizedPatchCover\_of\_twoCharts} takes the
identity and inversion realized patches explicitly together with their
two-patch-shaped bookkeeping (the source/target cover as binary disjunctions, the
per-patch \code{targetChart} agreements, the two standardness facts, and the two
\code{NormalizedChartBallLimit} nonemptiness facts) and packages the
selection-target existential with \(\code{PatchIndex}:=\code{Bool}\) (identity at
\code{false}, inversion at \code{true}). The \code{Fintype}/\code{Nonempty}
instances and the exhaustive alternative are synthesized for free by
\code{Bool} case analysis, and the source/target cover follows from the binary
disjunctions. Non-opaque (the patches are explicit) and not gated on the open
analytic spine; pure assembly.
\lean{JacobianChallenge.HolomorphicForms.exists_realizedPatchCover_of_twoCharts}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Coordinate convergence transfer]
\label{lem:montel-realized-patch-coord-convergence-transfer}
\uses{lem:montel-realized-patch-cover-packaging}
The green E\(\to\)F seam: pull chart-ball-domain locally-uniform convergence back
to patch-source locally-uniform convergence of the patch coordinate. The engine's
diagonal extraction produces, per patch, locally-uniform convergence of the
chart-ball readings on the chart-ball domain \(\code{ball center radius}\)
(in \(\C\)); the selection-target \(F\) clause instead needs locally-uniform
convergence of the patch \emph{coordinate} on \(\code{patch.source}\) (in \(X\)).
Given a realized patch and a family \(g:\N\to\C\to\C\) converging locally
uniformly to \(\code{localPatch.chartBall.toFun}\) on the chart-ball domain, the
precomposition \(\code{fun}\ n\ x\mapsto g\,n\,(\code{sourceChart}\,x)\) converges
locally uniformly to \(\code{patch.coord}\) on \(\code{patch.source}\). The proof
pulls back along the realization's \code{sourceChart} via Mathlib's
\code{TendstoLocallyUniformlyOn.comp} --- continuity from
\code{sourceChart\_contMDiffOn.continuousOn}, the \code{MapsTo} from
\code{sourceChart\_mem\_chartBall} --- then rewrites the limit
\(\code{chartBall.toFun}\circ\code{sourceChart}=\code{coord}\) on the source via
\code{coord\_eq\_chartBall}. Pure assembly over Mathlib and the realization
fields; not gated on the open spine (the chart-ball convergence is a hypothesis).
\lean{JacobianChallenge.HolomorphicForms.tendstoLocallyUniformlyOn_coord_of_chartBall}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Compact target-range package for the stage diagonal subsequence]
\label{lem:montel-stage-compact-target-package}
The un-gated assembly that produces the compact-target inputs of the stage common
diagonal subsequence. The green diagonal-extraction consumer takes target inputs
\(\code{target}\), \(\code{htarget}\) (compactness), and \(\code{hrange}\) (the
bounded closed-ball readings land in \(\code{target}\)). From a single closed-ball
value bound \(B:\iota\to\R\) on the finite stage family
\(\code{data}:\iota\to\N\to\code{ChartBallPowerSeries}\), this package supplies all
three: \(\code{target}\,i:=\code{closedBall}\,(0:\C)\,(B\,i)\), with compactness
from \code{isCompact\_closedBall} and the range membership reducing --- via
\code{boundedContinuousOnClosedBall\_apply} and \code{mem\_closedBall\_zero\_iff}
--- to the value bound \(\|(\code{data}\,i\,n).\code{toFun}\,z\|\le B\,i\). The
value bound is a \emph{hypothesis}: this package neither produces the bound (the
engine spine does, once the holomorphic stage coordinate exists) nor performs the
diagonal extraction; it is the un-gated step turning the bound into the consumer's
\(\code{target}/\code{htarget}/\code{hrange}\) argument slots.
\lean{JacobianChallenge.HolomorphicForms.stageChartReading_compactTarget_of_ballBound}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}

\begin{lemma}[Per-patch realization over the chart-ball-limit patch]
\label{lem:montel-realized-patch-ofChartBallLimit}
\uses{lem:montel-raw-source-chart-package}
The green seam just upstream of the raw-chart realization corollary. Per patch,
the engine produces a normalized chart-ball limit, promotes it to a
\code{LocalNormalizedChartHomeomorphData} (the F2 local-chart leaf), and assembles
a \code{GenusZeroLocalMontelChartPatch} via
\code{GenusZeroLocalMontelChartPatch.ofChartBallLimit}; only then is the patch
realized. This corollary composes the \code{ofChartBallLimit} assembly with the
raw-chart M-D realization into one call: given the chart-ball power series, its
local normalized chart-homeomorphism data, a standard target chart, and a raw
source chart \(\varphi\) satisfying the M-D hypotheses for the assembled patch, it
produces \(\code{Nonempty}\,(\code{MontelRealizedPatch}\,X\,(\code{ofChartBallLimit}\,\dots))\).
The local-chart datum and raw chart are taken \emph{explicitly} (as in the
cover-packaging adapter) so the caller controls the witnesses the
\code{hsymm\_image} hypothesis is stated against; since \code{ofChartBallLimit}
sets its fields definitionally, the M-D hypotheses transfer by \code{rfl} with no
restatement. Pure assembly --- no analytic content, not gated on the open spine.
Only the explicit-\code{localChart} form is provided; the optional
convergence-input variant (threading the F2 leaf
\code{exists\_localNormalizedChartHomeomorphData\_of\_tendstoLocallyUniformlyOn}
via \code{.some}) was dropped because that \code{.some} makes the resulting
\code{localOpen} opaque, so the M-D \code{hsymm\_image} hypothesis could not be
stated cleanly against it --- the explicit form keeps the witnesses concrete.
\lean{JacobianChallenge.HolomorphicForms.exists_montelRealizedPatch_ofChartBallLimit}
\leanok
\end{lemma}
\begin{proof}\leanok\end{proof}
...
\begin{lemma}[Stage marked topological control data]
...
\lean{JacobianChallenge.HolomorphicForms.GenusZeroStageMarkedData, JacobianChallenge.HolomorphicForms.genusZeroStageMarkedData_nonempty}
\leanok
\end{lemma}
\begin{proof}\leanok
Take \(P_0 := e^{-1}(0)\), \(P_\infty := e^{-1}(\infty)\), and base point
\(e^{-1}(1)\); distinctness follows from injectivity of \(e^{-1}\), and the
three neighborhoods are pairwise \(T_2\) separations intersected with the
chart sources.
\end{proof}

\begin{lemma}[Stage dipole local singular profiles]
\label{lem:stage-dipole-local-singular-profiles}
\uses{lem:stage-marked-topological-control-data}
Every stage marked-data package carries local logarithmic singular profiles
at its two marked ends: functions \(u_0, u_\infty : X \to \mathbb{R}\) with
logarithmic singularity of sign \(+1\) at \(P_0\) and \(-1\) at
\(P_\infty\), each continuous on the punctured chart source around its
singular point — hence on the punctured marked neighborhoods \(U_0\),
\(U_\infty\).  This packages the model-plane and chart-local dipole leaves
as boundary-data germs for the stage construction; the bordered-stage
boundary problem itself (Lemma~\ref{lem:stage-dipole-boundary-control})
remains open engine work.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroStageDipoleProfiles, JacobianChallenge.HolomorphicForms.genusZeroStageDipoleProfiles_nonempty}
\leanok
\end{lemma}
\begin{proof}\leanok
Take \(u_0(x) := \log\lVert\varphi_{P_0}(x) - \varphi_{P_0}(P_0)\rVert\) for
the chart \(\varphi_{P_0}\) at \(P_0\) and
\(u_\infty := -\log\lVert\varphi_{P_\infty}(\cdot) -
\varphi_{P_\infty}(P_\infty)\rVert\); singularities by the chart-pullback
lift of the model-plane log witnesses, continuity off the singular point
from chart continuity, chart injectivity on the source, and continuity of
\(\log\) away from \(0\).
\end{proof}

\begin{lemma}[Stage dipole glued potential]
\label{lem:stage-dipole-glued-potential}
\uses{lem:stage-marked-topological-control-data, lem:stage-dipole-local-singular-profiles}
One potential carries the boundary-control core simultaneously: gluing the
\(+1\) profile on \(U_0\), the \(-1\) profile on \(U_\infty\), and \(0\)
elsewhere (well-defined since the marked neighborhoods are pairwise
disjoint) yields \(u : X \to \mathbb{R}\) vanishing at the base point,
with logarithmic singularities \(+1\) at \(P_0\) and \(-1\) at
\(P_\infty\), agreeing with the local profiles on the punctured marked
neighborhoods.  The singularities transfer because the logarithmic
singularity condition is a germ predicate at the marked point.  This is
the stage-independent core of the boundary-control obligation
(Lemma~\ref{lem:stage-dipole-boundary-control}); only the per-stage
boundary-chart and compact-subdomain bounds remain there.
\lean{JacobianChallenge.HolomorphicForms.StageDipolePotentialData, JacobianChallenge.HolomorphicForms.stageDipolePotentialData_nonempty, JacobianChallenge.HolomorphicForms.HasLogarithmicSingularityAtReal.congr_on_nhds}
\leanok
\end{lemma}
\begin{proof}\leanok
Case-glue over the pairwise disjoint neighborhoods; base normalization by
disjointness with \(U_{\mathrm{base}}\); agreement is definitional on each
neighborhood; singularities via the germ-congruence helper applied to the
neighborhood agreements (the chart inverse is continuous at the chart
image of the marked point, so neighborhood agreement upgrades to eventual
equality of the chart-pulled-back integrands).
\end{proof}

\begin{lemma}[Stage log-profile off-point conjugates]
\label{lem:stage-log-profile-off-point-conjugates}
\uses{lem:stage-dipole-local-singular-profiles, lem:stage-dipole-glued-potential}
The canonical chart-pullback log profiles
\(y \mapsto \pm\log\lVert\varphi_P(y) - \varphi_P(P)\rVert\) admit local
harmonic conjugates at every point of their punctured chart sources, and
hence the glued stage dipole potential over the canonical profiles admits
local harmonic conjugates at every point of the punctured marked
neighborhoods \(U_0 \setminus \{P_0\}\) and
\(U_\infty \setminus \{P_\infty\}\) — the
\code{StageHarmonicOn}-shaped facts the Dirichlet-stage interface
demands on the marked neighborhoods.  Single-log mirror of the dipole
conjugate path: one pole, one chart transition, a single-point slit
rotation.
\lean{JacobianChallenge.HolomorphicForms.chart_pullback_log_has_conjugate_at_off_P, JacobianChallenge.HolomorphicForms.chart_pullback_neg_log_has_conjugate_at_off_P, JacobianChallenge.HolomorphicForms.stageDipoleGluedPotential_canonical_has_conjugate_on_U0, JacobianChallenge.HolomorphicForms.stageDipoleGluedPotential_canonical_has_conjugate_on_Uinf}
\leanok
\end{lemma}
\begin{proof}\leanok
Re-express the pullback log through the chart at the evaluation point
(germ equality on the chart source); the transition
\(\varphi_P \circ \varphi_x^{-1}\) is analytic on the overlap, nonzero
after subtracting the marked value (chart injectivity), and rotates into
the slit plane, so \(\log(c\,h_P(\cdot)) - \log\lVert c\rVert\) is an
analytic witness whose imaginary part \(\arg(c\,h_P(\cdot))\) is the
conjugate; lift through the chart at the basepoint and transfer along the
germ equality.  The glued-potential corollaries transfer along the
full-neighborhood agreements of the case-glue.
\end{proof}

\begin{lemma}[Stage log-profile chartwise harmonic readings]
\label{lem:stage-log-profile-chartwise-harmonic-readings}
\uses{lem:stage-log-profile-off-point-conjugates, lem:stage-harmonic-analytic-compose}
The canonical chart-pullback log profiles, and the glued stage dipole
potential over them, have \emph{Mathlib-harmonic chart readings}: around
every point of the punctured chart sources (resp.\ the punctured marked
neighborhoods \(U_0 \setminus \{P_0\}\), \(U_\infty \setminus
\{P_\infty\}\)) there is a preferred-chart ball, contained in the chart
target and reading into the chart source (resp.\ into \(U_0\),
\(U_\infty\)), on which the chart reading of the potential is harmonic in
the sense of \code{InnerProductSpace.HarmonicOnNhd}.  This is the
B1-layer feedstock in the repaired Dirichlet payload's export shape
(\code{StageHarmonicOnNhd}): the stage-assembly and conjugate lanes
consume chartwise harmonicity directly instead of re-deriving it from
the per-point conjugate witnesses.
\lean{JacobianChallenge.HolomorphicForms.harmonicAt_log_pullback_reading, JacobianChallenge.HolomorphicForms.exists_chartBall_harmonicOnNhd_log_pullback, JacobianChallenge.HolomorphicForms.exists_chartBall_harmonicOnNhd_neg_log_pullback, JacobianChallenge.HolomorphicForms.canonical_u0_harmonicOnNhd_chartBall, JacobianChallenge.HolomorphicForms.canonical_uinf_harmonicOnNhd_chartBall, JacobianChallenge.HolomorphicForms.stageDipoleGluedPotential_canonical_harmonicOnNhd_on_U0, JacobianChallenge.HolomorphicForms.stageDipoleGluedPotential_canonical_harmonicOnNhd_on_Uinf}
\leanok
\end{lemma}
\begin{proof}\leanok
The chart reading of the pullback log is definitionally
\(\log\lVert(\varphi_P \circ \varphi_x^{-1})(\cdot) -
\varphi_P(P)\rVert\); the transition is analytic on the overlap and the
subtracted value is nonzero off the marked point by chart injectivity,
so \(\log\lVert f\rVert\)-harmonicity for nonvanishing analytic \(f\)
applies pointwise.  The ball is extracted from the open set carved by
the chart target, the source preimage, and the \emph{transition-value}
puncture \((\varphi_P \circ \varphi_x^{-1})(\cdot) \ne \varphi_P(P)\)
(no Hausdorff hypothesis needed); the \(-1\) profile negates pointwise,
and the glued-potential versions shrink the ball into the marked
neighborhood and transfer along the case-glue agreement by eventual
equality.
\end{proof}
...
controlled by finitely many source charts, each boundary piece lying inside
the stage frontier (the S1 statement repair: pieces covering interior points
would let boundary data overconstrain the harmonic solution on an open set,
refuting the Dirichlet node).  These domains are the non-circular
places where the analytic stage maps will be constructed before being extended
by filler values outside \(\Omega_n\).
\lean{JacobianChallenge.HolomorphicForms.StageBorderedExhaustion, JacobianChallenge.HolomorphicForms.exists_stageBorderedExhaustion}
\end{lemma}
...
cuts and contained in the holomorphic part of the stage, with the marked ends
excluded from every cut domain and the base point eventually retained there.
\lean{JacobianChallenge.HolomorphicForms.StageCutSystem, JacobianChallenge.HolomorphicForms.StageMarkedCutSystem, JacobianChallenge.HolomorphicForms.StageConjugateReady, JacobianChallenge.HolomorphicForms.exists_stageCutSystem}
\end{lemma}

\begin{lemma}[Stage eventual-containment bounds]
\label{lem:stage-eventual-containment-bounds}
\uses{lem:stage-bordered-exhaustion-domains, lem:stage-cut-domain-conjugate-control}
The filter-level containment and avoidance promises of the exhaustion and
cut-system interfaces yield the concrete bounds the analytic stage
consumers invoke: for each selected compactum a threshold \(N\) past which
it is contained in the stage (resp.\ contained in the cut domain and
disjoint from the cuts, simultaneously); a \emph{uniform} such threshold
for any finite family of selected compacta; and subset wrappers so closed
coordinate subballs inherit all bounds.  Pure filter algebra over the
interface fields — independent of how the open exhaustion and cut-system
obligations are eventually discharged.
\lean{JacobianChallenge.HolomorphicForms.StageBorderedExhaustion.exists_uniform_stage_bound, JacobianChallenge.HolomorphicForms.StageCutSystem.exists_uniform_contains_and_avoids_bound, JacobianChallenge.HolomorphicForms.StageCutSystem.exists_contains_and_avoids_bound_of_subset}
\leanok
\end{lemma}
\begin{proof}\leanok
Unfold \(\forall^f n\) at \texttt{atTop} via \texttt{eventually\_atTop};
conjoin containment and avoidance with \texttt{Eventually.and}; pass to
finite families with \texttt{eventually\_all\_finset}; subset wrappers are
transitivity of \(\subseteq\) and \texttt{Disjoint.mono\_left}.
\end{proof}

\begin{lemma}[Stage dipole boundary control (open)]
\label{lem:stage-dipole-boundary-control}
\uses{lem:stage-marked-topological-control-data, lem:stage-bordered-exhaustion-domains, lem:stage-dipole-local-singular-profiles, lem:stage-dipole-glued-potential}
The stage boundary data is built from logarithmic dipole models at the two
marked ends, with boundary normalization, continuity of the boundary datum on
each stage frontier (the S6 statement repair: discontinuous boundary data
admits no harmonic comparison theory and made the Dirichlet node refutable),
and compact-subdomain bounds suitable
...
\lean{JacobianChallenge.HolomorphicForms.StageDipoleBoundaryControl, JacobianChallenge.HolomorphicForms.exists_stageDipoleBoundaryControl}
\end{lemma}

\begin{lemma}[Stage Dirichlet harmonic solution (open)]
\label{lem:stage-dirichlet-harmonic-exists}
\uses{lem:stage-bordered-exhaustion-domains, lem:stage-dipole-boundary-control, lem:stage-harmonic-max-principle, def:perron-sub-on, lem:harmonic-disc-comparison, lem:perron-harnack-harmonic-limit, lem:perron-sub-on-harmonic-member}
On each stage domain of the holomorphic atlas (the manifold hypothesis is the
S5 statement repair: chartwise harmonicity transfers between preferred charts
only along holomorphic transitions), solve the normalized Dirichlet/Perron
problem for
the real harmonic dipole potential with the prescribed boundary control and
marked-end singular behavior.  The payload exports chartwise
neighborhood-uniform harmonicity of the preferred-chart readings (Mathlib's
\texttt{HarmonicOnNhd}; per-point conjugate witnesses are a derived bridge,
the S2/R2 statement repair), and a maximum-principle bracket trapping the
solution between the frontier infimum and supremum of the boundary datum up
to a per-stage constant away from the marked neighborhoods (the boundary
lever, replacing pointwise frontier agreement: exact boundary attainment
would cost the Poisson boundary-limit theorem plus a barrier subtree, and
downstream consumers need only the bracket together with the exact
logarithmic germs).  Base normalization is retained: with the bracket's
slack absorbing constant shifts, it no longer over-determines the problem.
This is the load-bearing
potential-theory frontier of the engine: Mathlib supplies harmonic predicates
and Poisson formulas for already-harmonic functions, but not this
bordered-stage existence theorem.
\lean{JacobianChallenge.HolomorphicForms.StageDirichletHarmonicSolution, JacobianChallenge.HolomorphicForms.exists_stageDirichletHarmonicSolution}
\end{lemma}
...
single-valued on the chosen cut stage.  Stated over the holomorphic atlas
(the S5 hypothesis inherited from the Dirichlet node).
\lean{JacobianChallenge.HolomorphicForms.StageHarmonicConjugatesOnCuts, JacobianChallenge.HolomorphicForms.exists_stageHarmonicConjugatesOnCuts}
\end{lemma}
...
compactness extraction.  Stated over the holomorphic atlas (the S5
hypothesis inherited from the Dirichlet node).
\lean{JacobianChallenge.HolomorphicForms.StageHolomorphicCoordinates, JacobianChallenge.HolomorphicForms.exists_stageHolomorphicCoordinatesFromHarmonicDipole}
\end{lemma}
...
\lean{JacobianChallenge.HolomorphicForms.StageCoordinateChartNormalization, JacobianChallenge.HolomorphicForms.StageCoordinateInvertedChartNormalization, JacobianChallenge.HolomorphicForms.StageCoordinateNoncollapse, JacobianChallenge.HolomorphicForms.StageNormalizedHolomorphicCoordinates, JacobianChallenge.HolomorphicForms.exists_stageNormalizedHolomorphicCoordinates}
\end{lemma}

\begin{lemma}[Stage harmonic maximum principle]
\label{lem:stage-harmonic-max-principle}
On a bounded planar domain, the real part of a holomorphic function
continuous up to the closure is bounded on the closure by any bound valid
on the frontier: \(\lVert e^{f}\rVert = e^{\operatorname{re} f}\) reduces
the claim to the maximum modulus principle, and monotonicity of the real
exponential transfers it back.  Two-sided, closed-disc/boundary-circle,
and conjugate-pair (\(u + iv\) holomorphic \(\Rightarrow\) frontier bound
controls \(u\)) forms are provided — the shapes in which the stage
Dirichlet solutions and the uniform Cauchy-circle estimates consume
boundedness.  Pure model-plane complex analysis; no stage dependencies.
A genuinely harmonic form is also provided (W1): the weak maximum
principle for \texttt{HarmonicOnNhd} on a bounded open set — frontier
nonpositivity propagates inside via the mean-value property, local
maximum propagation, and a clopen argument over the connected component
(boundedness is load-bearing: the component cannot be clopen in
\(\mathbb{C}\)) — discharging the frozen
\texttt{WeakMaxPrincipleInput} obligation of the B2 toolbox.
\lean{JacobianChallenge.HolomorphicForms.re_le_of_forall_mem_frontier_re_le, JacobianChallenge.HolomorphicForms.re_le_on_closedBall_of_sphere_re_le, JacobianChallenge.HolomorphicForms.re_bound_of_conjugate_pair, JacobianChallenge.HolomorphicForms.eq_const_closedBall_of_harmonicOnNhd_of_le, JacobianChallenge.HolomorphicForms.le_zero_of_harmonicOnNhd_of_frontier_le_zero, JacobianChallenge.HolomorphicForms.weakMaxPrincipleInput_holds}
\leanok
\end{lemma}
\begin{proof}\leanok
Apply \texttt{Complex.norm\_le\_of\_forall\_mem\_frontier\_norm\_le} to
\(\exp \circ f\) (entire composed with \texttt{DiffContOnCl}); its norm is
\(\exp(\operatorname{re} f)\), and \(\exp\) is monotone.  The disc form
uses \(\partial B = S\) and \(\overline{B} = \bar{B}\) for positive
radius; the conjugate-pair form is the special case
\(\operatorname{re}(u + iv) = u\).
\end{proof}

\begin{lemma}[Stage harmonicity composes with analytic maps]
\label{lem:stage-harmonic-analytic-compose}
Precomposing a harmonic function with a complex-analytic map preserves
harmonicity: if \(u\) is harmonic at \(f(z_0)\) and \(f\) is analytic at
\(z_0\), then \(u \circ f\) is harmonic at \(z_0\); likewise on sets, and
for the chart transitions
\(\varphi_P \circ \varphi_x^{-1}\) of the stage construction on their
overlap domains.  This is the chart-transfer workhorse for stage
potentials: maximum-principle bounds, conjugate transfer, and the Perron
subfunction API move potentials between source charts through exactly
these compositions.
\lean{JacobianChallenge.HolomorphicForms.HarmonicAt.comp_analyticAt, JacobianChallenge.HolomorphicForms.HarmonicOnNhd.comp_analyticOnNhd, JacobianChallenge.HolomorphicForms.HarmonicAt.comp_chart_transition}
\leanok
\end{lemma}
\begin{proof}\leanok
Harmonicity at a point holds on an open set, so on a ball around
\(f(z_0)\) the potential is the real part of an analytic \(F\); then
\(F \circ f\) is analytic at \(z_0\), its real part is harmonic, and
\(u \circ f\) agrees with it near \(z_0\) by continuity of \(f\) into the
ball, so harmonicity transfers along the germ equality.  The chart form
extracts analyticity of the transition from its smoothness on the
overlap.
\end{proof}

\begin{lemma}[Harnack inequality on discs]
\label{lem:perron-harnack-disc}
For \(h\) harmonic on a neighborhood of \(\overline{B}(c, R)\),
nonnegative on the boundary circle, and \(w\) in the open ball with
\(r = \lVert w - c\rVert\), the two-sided Harnack inequality holds:
\[
  \frac{R - r}{R + r}\, h(c) \;\le\; h(w) \;\le\; \frac{R + r}{R - r}\, h(c).
\]
Restated Poisson-kernel bounds on the integration circle are provided as
reusable leaves.  This is the Perron-engine B2 toolbox item W4
(inequality half); the increasing-limit corollary consumed by the Perron
envelope builds on it.  Pure model-plane harmonic analysis; no stage
dependencies.
\lean{JacobianChallenge.HolomorphicForms.harnack, JacobianChallenge.HolomorphicForms.harnack_upper, JacobianChallenge.HolomorphicForms.harnack_lower, JacobianChallenge.HolomorphicForms.poissonKernel_le_of_mem_sphere, JacobianChallenge.HolomorphicForms.le_poissonKernel_of_mem_sphere}
\leanok
\end{lemma}
\begin{proof}\leanok
Represent \(h(w)\) by the Poisson integral
(\texttt{HarmonicOnNhd.circleAverage\_poissonKernel\_smul}), bound the
kernel on the circle by \((R \pm r)/(R \mp r)\) via the Herglotz--Riesz
estimates (\texttt{re\_herglotzRieszKernel\_le},
\texttt{le\_re\_herglotzRieszKernel}), multiply by \(h \ge 0\) on the
circle, and compare averages (\texttt{circleAverage\_mono}); the
constant-scaled average collapses to \(h(c)\) by the mean-value property.
\end{proof}

\begin{lemma}[Disc Dirichlet solution: boundary attainment and packaging (W3d)]
\label{lem:perron-poisson-boundary-limit}
\uses{lem:perron-harnack-disc}
For boundary data \(\varphi\) continuous on the circle
\(\partial B(c, R)\), \(R > 0\), the Poisson extension attains
\(\varphi\) at every boundary point: \(P[\varphi](w) \to \varphi(\zeta)\)
as \(w \to \zeta\) within the open ball
(\texttt{poissonOperator\_tendsto\_of\_continuousOn}, the Schwarz
boundary-limit theorem, continuous data only), and the glued function —
Poisson extension inside the open ball, boundary datum outside — is
continuous on the closed ball and harmonic on the open ball: the
solution of the disc Dirichlet problem.  This is the B2 toolbox row
W3d, revived at the W5b/W6 boundary-attainment gap; the
consumption-shaped corollaries live in the consuming lanes.
\lean{JacobianChallenge.HolomorphicForms.poissonSolution, JacobianChallenge.HolomorphicForms.poissonSolution_apply_of_mem_ball, JacobianChallenge.HolomorphicForms.poissonSolution_apply_of_mem_sphere, JacobianChallenge.HolomorphicForms.poissonSolution_harmonicOnNhd, JacobianChallenge.HolomorphicForms.continuousOn_poissonSolution}
\leanok
\end{lemma}
\begin{proof}\leanok
Approximate identity on the circle.  Far from \(\zeta\) the kernel mass
dies uniformly as the pole approaches \(\zeta\)
(\texttt{tendstoUniformlyOn\_poissonKernel\_far}: the numerator
\(R^2 - \lVert w - c\rVert^2\) vanishes against a denominator kept
\(\ge (\delta/2)^2\) by the triangle inequality through \(\zeta\));
near \(\zeta\) the oscillation of \(\varphi\) rides the normalized
kernel weight.  The two combine in the pointwise integrand majorant
\(|P[\varphi](w) - \varphi(\zeta)| \le \eta + 2M\kappa\)
(\texttt{abs\_poissonOperator\_sub\_le}, no domain splitting, evaluated
exactly by the W3b normalization), and \(\varepsilon\)-bookkeeping
gives the boundary limit.  Continuity of the glued solution at a
boundary point splits the closed-ball neighborhood filter along
\(B \cup \partial B = \overline{B}\): the interior approach is the
boundary limit, the along-the-circle approach is continuity of
\(\varphi\).  Interior continuity and harmonicity transfer from the
W3a operator along germ equality on the open ball.
\end{proof}

\begin{lemma}[Harnack convergence, uniform-limit half]
\label{lem:perron-harnack-uniform-limit}
\uses{lem:perron-harnack-disc}
An increasing sequence of harmonic functions on a neighborhood of
\(\overline{B}(c, R)\), bounded at the center, converges pointwise on
the open ball and uniformly on every closed disc
\(\overline{B}(c, \rho)\) with \(\rho < R\).  The constant in the
Harnack upper bound is taken uniformly over the inner disc
(\(\le (R+\rho)/(R-\rho)\)), so the differences \(h_n - h_m \ge 0\)
have oscillation controlled by their value at the center, which is
Cauchy by monotone boundedness.  This is the convergence half of the
W4 increasing-limit corollary the Perron envelope consumes;
harmonicity of the limit (via the Poisson operator, W3a) is the
separate second half.
\lean{JacobianChallenge.HolomorphicForms.harnack_upper_on_closedBall, JacobianChallenge.HolomorphicForms.harnack_increasing_tendstoUniformlyOn}
\leanok
\end{lemma}
\begin{proof}\leanok
Monotonize the Harnack constant over \(w \in \overline{B}(c, \rho)\)
(cross-multiplied monotonicity of \((R+r)/(R-r)\) in \(r\), with
\(h(c) \ge 0\) from the mean-value property).  Pointwise limits exist
on the open ball by monotone convergence against the Harnack bound on
\(h_n - h_0\); passing the per-pair difference bound to the limit in
\(k\) gives \(H(w) - h_n(w) \le \frac{R+\rho}{R-\rho}\,(H(c) - h_n(c))\)
uniformly on \(\overline{B}(c, \rho)\), and the right side tends to
\(0\).
\end{proof}

\begin{lemma}[Harnack convergence: the limit is harmonic]
\label{lem:perron-harnack-harmonic-limit}
\uses{lem:perron-harnack-uniform-limit}
A sequence of functions harmonic on a neighborhood of
\(\overline{B}(c, R)\) converging uniformly on every closed disc
\(\overline{B}(c, \rho)\), \(\rho < R\), has a harmonic limit on the
open ball — no monotonicity required.  Combined with the uniform-limit
half this completes the W4 increasing-limit corollary in the exact form
the Perron envelope (W7) consumes: increasing, harmonic, bounded at the
center \(\Rightarrow\) pointwise and locally uniform convergence to a
harmonic limit.  The route avoids the mean-value converse (absent from
Mathlib): the limit is exhibited in Poisson-operator form and
harmonicity in the pole is the W3a theorem
(\texttt{poissonOperator\_harmonicOnNhd}).
\lean{JacobianChallenge.HolomorphicForms.harmonicOnNhd_ball_of_tendstoUniformlyOn, JacobianChallenge.HolomorphicForms.harnack_increasing_limit_harmonic}
\leanok
\end{lemma}
\begin{proof}\leanok
At \(w_0\) in the open ball pick
\(\rho = (\lVert w_0 - c\rVert + R)/2\).  On \(\overline{B}(c, \rho)\)
each \(h_n\) equals its Poisson integral
(\texttt{HarmonicOnNhd.circleAverage\_poissonKernel\_smul}); the
operator is Lipschitz in the boundary datum
(\(|P[\varphi] - P[\psi]| \le \sup_{\partial}|\varphi - \psi|\), from
linearity and the W3c brackets), so uniform convergence on the circle
passes the identity to the limit: \(H = P[H]\) on \(B(c, \rho)\).  The
right side is harmonic in the pole (W3a), and harmonicity transfers
along the germ equality at \(w_0\).
\end{proof}

\begin{lemma}[Perron envelope: center-touch core]
\label{lem:perron-envelope-center-touch}
\uses{lem:perron-harnack-harmonic-limit, def:perron-sub-on}
Define the Perron envelope of a family \(F\) of real functions as the
pointwise supremum.  For \(F\) nonempty, closed under pointwise
\(\max\), pointwise bounded above on \(V\), and admitting Poisson
modification on a disc \(\overline{B}(c, R) \subseteq V\) (every member
is dominated by a member harmonic on \(B(c, R)\) — the W5b/W3d-gated
capability, consumed here through the frozen obligation
\(\texttt{PoissonModificationInput}\)): through every point \(x_0\) of
the open disc there is a function harmonic on \(B(c, R)\), lying below
the envelope, and equal to the envelope at \(x_0\).  This is the
constructive half of Perron's lemma (W7); the second-comparison step
upgrading the touch to equality throughout the disc is the next node.
An open-ball version of the W4 Harnack convergence corollary is
provided alongside (the modification oracle produces functions harmonic
on the open ball only).
\lean{JacobianChallenge.HolomorphicForms.perronEnvelope, JacobianChallenge.HolomorphicForms.PoissonModificationInput, JacobianChallenge.HolomorphicForms.harnack_increasing_limit_harmonic_ball, JacobianChallenge.HolomorphicForms.perronEnvelope_exists_harmonic_eq_at}
\leanok
\end{lemma}
\begin{proof}\leanok
Choose members \(v_n\) with
\(u(x_0) - \tfrac{1}{n+1} < v_n(x_0)\) (supremum approximation),
interleave \(\max\)-monotonization with Poisson modification to get an
increasing sequence of members harmonic on the disc, squeezed at
\(x_0\) and bounded at the center by the envelope.  The W4 Harnack
convergence corollary (open-ball version, by shrinking and gluing the
radius-independent pointwise-sup limits) yields a harmonic limit \(H\);
\(H \le u\) since every term is a member, and \(H(x_0) = u(x_0)\) by
the squeeze.
\end{proof}

\begin{lemma}[Poisson-modification oracle for PerronSubOn families]
\label{lem:perron-envelope-poisson-modification-input}
\uses{lem:perron-envelope-center-touch, lem:perron-sub-on-poisson-modify, lem:m4-poisson-solution}
The frozen obligation \(\texttt{PoissonModificationInput}(F, V, c, R)\) holds
for any family \(F\) whose members are all \(\texttt{PerronSubOn}(V)\) and
which is closed under the disc Poisson modification
(\(v \in F \Rightarrow \texttt{poissonSolution}\,v\,c\,R \in F\)), given a
disc \(\overline{B}(c,R) \subseteq V\).  The witness for a member \(v\) is
its own W5b-iii modification \(\texttt{poissonSolution}\,v\,c\,R\):
membership is the closure hypothesis; domination
\(v \le \texttt{poissonSolution}\,v\,c\,R\) on \(V\) is the W5b-iii
comparison half inside \(B(c,R)\) and equality outside; disc-harmonicity is
the M4 interior conclusion.  This discharges the oracle the §3.5 envelope
assembly and the center-touch core consume, for exactly the family shape
they build.  The modification-closure clause is load-bearing;
\(\max\)-closure enters at the consumer, not here.  No maximum-principle
obligation (the oracle asks only for domination and harmonicity, not
membership of the witness in \(\texttt{PerronSubOn}\)).  \(\texttt{PerronEnvelope}\)
is consumed by import only, never edited.
\lean{JacobianChallenge.HolomorphicForms.poissonModificationInput_of_perronSubOn}
\leanok
\end{lemma}

\begin{lemma}[Strong maximum principle on balls, from Harnack]
\label{lem:harnack-strong-max-ball}
\uses{lem:perron-harnack-disc}
A harmonic \(g \le 0\) on \(B(c, R)\) vanishing at one interior point
vanishes identically; equivalently (the consumed form), two harmonic
functions on a ball with \(h_1 \le h_2\) throughout and equality at
one interior point are equal on the whole ball.  This is the
interior-rigidity input for the Perron envelope's second-comparison
step (W7-ii); the weak maximum principle (W1) cannot supply it, since
boundary bounds do not see a touching point.  Pure W4a corollary —
no W1 dependency.
\lean{JacobianChallenge.HolomorphicForms.harnack_strong_max_ball, JacobianChallenge.HolomorphicForms.eq_on_ball_of_harmonic_le_of_eq_at}
\leanok
\end{lemma}
\begin{proof}\leanok
Harnack propagation: at an interior zero \(z_0\), the upper Harnack
bound for \(-g \ge 0\) on the sub-disc of radius
\((R - \lVert z_0 - c\rVert)/2\) gives
\((-g)(w) \le C(w) \cdot (-g)(z_0) = 0\), so the zero set contains a
ball around each of its points; the strictly negative locus is open by
continuity; the ball is preconnected
(\texttt{Convex.isPreconnected}), so a nonempty open partition is
impossible and the zero set is everything.  The two-function form
applies this to \(h_1 - h_2\).
\end{proof}

\begin{lemma}[Perron's lemma: the envelope is harmonic]
\label{lem:perron-envelope-harmonic}
\uses{lem:perron-envelope-center-touch, lem:harnack-strong-max-ball, lem:perron-harnack-harmonic-limit}
For a nonempty family \(F\) closed under pointwise \(\max\), pointwise
bounded above on \(V\), with the Poisson modification capability on a
disc \(\overline{B}(c, R) \subseteq V\): the Perron envelope
\(u = \sup F\) is harmonic on \(B(c, R)\).  This is the W7 planar
envelope lemma — the engine that makes the Green's-function and
\(H[g]\) suprema harmonic (§3.3–3.4) — in exactly the shape frozen as
\(\texttt{PerronEnvelopeHarmonicInput}\) by the W9 consumer.  No
\(\texttt{PerronSubOn}\) hypothesis is needed: boundedness and
membership do all the bounding.
\lean{JacobianChallenge.HolomorphicForms.perronEnvelope_harmonicOnNhd_ball}
\leanok
\end{lemma}
\begin{proof}\leanok
Run the center-touch construction at the center \(c\): a modified
increasing member sequence \(g_n\) with harmonic limit \(H \le u\),
\(H(c) = u(c)\).  For arbitrary \(y \in B(c,R)\), run a second pass
seeded by \(\max(b_n, g_n)\) with \(b_n\) approximants at \(y\): the
harmonic limit \(K\) dominates \(H\), touches \(u\) at \(y\), and
agrees with \(H\) at \(c\) (squeezed between \(H(c)\) and \(u(c)\)).
Interior-touch rigidity (\(H \le K\), equal at \(c\)) forces
\(H \equiv K\), so \(u(y) = K(y) = H(y)\); as \(y\) was arbitrary,
\(u = H\) on the disc and harmonicity transfers along the germ
equality.
\end{proof}

\begin{definition}[Perron comparison subclass]
\label{def:perron-sub-on}
\uses{lem:stage-harmonic-analytic-compose}
For \(v : \mathbb{C} \to \mathbb{R}\) continuous on an open
\(V \subseteq \mathbb{C}\), say \(v\) is \(\texttt{PerronSubOn}(V)\) iff on every
closed disc \(\overline{B}(c, R) \subseteq V\) with \(R > 0\) and every
\(h\) harmonic on \(B(c, R)\) and continuous on \(\overline{B}(c, R)\)
with \(v \le h\) on the circle \(\partial B(c, R)\), one has \(v \le h\)
throughout \(B(c, R)\).  This is the continuous comparison subclass that
replaces upper-semicontinuous subharmonicity in Perron's method (B2
toolbox item W5): no Riesz representation, polar sets, or fine topology
are needed, because nothing in the genus-zero engine consumes the full
subharmonic theory.  The class is closed under \(\max\); harmonic
functions are members and the class is closed under Poisson modification
(those two facts consume the harmonic maximum principle and the Poisson
operator, deferred to the gated W5b leaf).  The comparison majorant is
packaged with Mathlib's \texttt{HarmonicContOnCl}.
\lean{JacobianChallenge.HolomorphicForms.PerronSubOn, JacobianChallenge.HolomorphicForms.PerronSubOn.max, JacobianChallenge.HolomorphicForms.PerronSubOn.continuousOn, JacobianChallenge.HolomorphicForms.PerronSubOn.le_on_ball, JacobianChallenge.HolomorphicForms.PerronSubOn.mono, JacobianChallenge.HolomorphicForms.PerronSubOn.congr}
\leanok
\end{definition}

\begin{lemma}[W1: harmonic weak maximum principle (green leaf)]
\label{lem:w1-weak-max-principle}
jc3's W1 M2 discharge: the harmonic weak maximum principle on a bounded
open \(V \subseteq \mathbb{C}\) — nonpositive frontier values force
nonpositivity inside — proved sorry-free, packaged as the discharge of the
frozen obligation \(\texttt{WeakMaxPrincipleInput}\).  Cited as an
already-green project leaf so the W5b adapters terminate in a proven node.
\lean{JacobianChallenge.HolomorphicForms.weakMaxPrincipleInput_holds, JacobianChallenge.HolomorphicForms.le_zero_of_harmonicOnNhd_of_frontier_le_zero}
\leanok
\end{lemma}

\begin{lemma}[Harmonic functions are Perron-comparison members]
\label{lem:perron-sub-on-harmonic-member}
\uses{def:perron-sub-on, lem:w1-weak-max-principle}
A function harmonic on an open \(V \subseteq \mathbb{C}\) is a
\(\texttt{PerronSubOn}(V)\) member.  On a disc \(\overline{B}(c,R) \subseteq V\)
with a harmonic-up-to-closure majorant \(h'\) dominating \(h\) on
\(\partial B(c,R)\), the difference \(w = h - h'\) is harmonic on
\(B(c,R)\), continuous on \(\overline{B}(c,R)\), and \(\le 0\) on
\(\partial B(c,R) = \operatorname{frontier} B(c,R)\); the weak maximum
principle forces \(w \le 0\) inside, i.e.\ \(h \le h'\).  This is the
basic membership fact the Green's-function and \(H[g]\) envelopes consume
(every envelope is a sup of members; harmonic functions are the basic
members).  The harmonic weak maximum principle is discharged by jc3's
\(\texttt{weakMaxPrincipleInput\_holds}\) (\ref{lem:w1-weak-max-principle});
the unconditional corollary \(\texttt{of\_harmonicOnNhd'}\) takes no
hypothesis, and the parametric original is retained for consumers that
thread a \(\texttt{WeakMaxPrincipleInput}\) term.
\lean{JacobianChallenge.HolomorphicForms.PerronSubOn.of_harmonicOnNhd, JacobianChallenge.HolomorphicForms.PerronSubOn.of_harmonicOnNhd'}
\leanok
\end{lemma}

\begin{lemma}[Two-harmonic disc comparison]
\label{lem:harmonic-disc-comparison}
\uses{def:perron-sub-on, lem:w1-weak-max-principle}
If \(h_1, h_2\) are harmonic on the open disc \(B(c,R)\), continuous up to
its closure, and \(h_1 \le h_2\) on the circle \(\partial B(c,R)\), then
\(h_1 \le h_2\) throughout \(B(c,R)\).  The difference \(w = h_1 - h_2\) is
harmonic on \(B(c,R)\), continuous on \(\overline{B}(c,R)\), and \(\le 0\)
on \(\partial B(c,R) = \operatorname{frontier} B(c,R)\); the weak maximum
principle forces \(w \le 0\) inside.  This is the applied disc form of the
maximum principle that the Green's-function upper bounds (§3.3) and the
Perron envelope locality step consume — the symmetric two-sided companion
to \ref{lem:perron-sub-on-harmonic-member}.  The harmonic weak maximum
principle is discharged by jc3's \(\texttt{weakMaxPrincipleInput\_holds}\)
(\ref{lem:w1-weak-max-principle}); the unconditional corollary
\(\texttt{harmonicContOnCl\_le\_of\_le\_on\_sphere'}\) takes no hypothesis,
and the parametric original is retained.
\lean{JacobianChallenge.HolomorphicForms.harmonicContOnCl_le_of_le_on_sphere, JacobianChallenge.HolomorphicForms.harmonicContOnCl_le_of_le_on_sphere'}
\leanok
\end{lemma}

\begin{lemma}[Perron subclass closed under adding a harmonic]
\label{lem:perron-sub-on-add-harmonic}
\uses{def:perron-sub-on}
If \(v\) is a \(\texttt{PerronSubOn}(V)\) member and \(h\) is harmonic on
the open set \(V\), then \(v + h\) is a \(\texttt{PerronSubOn}(V)\) member.
On a comparison disc \(\overline{B}(c,R) \subseteq V\) with majorant
\(h'\), the bound \(v + h \le h'\) on \(\partial B(c,R)\) rearranges to
\(v \le h' - h\), and \(h' - h\) is again harmonic-up-to-closure (\(h\)
restricts to the disc), so \(v\)'s comparison clause gives \(v \le h' - h\)
inside, i.e.\ \(v + h \le h'\).  This is the structural step the §3.3
Green's families (member plus the harmonic log profile) and the §3.5
envelope assembly consume; it subsumes the constant-shift case.  Purely
from the definition and closure of harmonicity-up-to-closure under
subtraction — no maximum principle.
\lean{JacobianChallenge.HolomorphicForms.PerronSubOn.add_harmonicOnNhd}
\leanok
\end{lemma}

\begin{lemma}[M4: disc Dirichlet packaging (green leaf)]
\label{lem:m4-poisson-solution}
The disc Dirichlet solution \(\texttt{poissonSolution}\,\varphi\,c\,R\) of
jc6's W3d M4: the Poisson extension of \(\varphi\) inside \(B(c,R)\), the
datum \(\varphi\) outside.  For \(\varphi\) continuous on the circle it is
harmonic on the open disc and continuous on the closed disc, and equals
\(\varphi\) on the circle.  Cited here as an already-green project leaf so
the Poisson-modification chain terminates in a proven node.
\lean{JacobianChallenge.HolomorphicForms.poissonSolution, JacobianChallenge.HolomorphicForms.poissonSolution_harmonicOnNhd, JacobianChallenge.HolomorphicForms.continuousOn_poissonSolution, JacobianChallenge.HolomorphicForms.poissonSolution_apply_of_mem_sphere}
\leanok
\end{lemma}

\begin{lemma}[Perron subclass closed under Poisson modification]
\label{lem:perron-sub-on-poisson-modify}
\uses{def:perron-sub-on, lem:m4-poisson-solution, lem:perron-sub-on-harmonic-member}
For a \(\texttt{PerronSubOn}(V)\) member \(v\) and a comparison disc
\(\overline{B}(c,R) \subseteq V\), the M4 Dirichlet solution
\(\texttt{poissonSolution}\,v\,c\,R\) — the Poisson modification of \(v\)
on that disc — is again a \(\texttt{PerronSubOn}(V)\) member.  Two facts
close the leaf.  First, \(v \le \texttt{poissonSolution}\,v\,c\,R\) on the
disc: the M4 extension is a legal harmonic-up-to-closure majorant whose
circle values are \(v\)'s own, so \(v\)'s comparison clause closes it with
no maximum-principle obligation.  Second, membership: on a test disc
\(\overline{B}(c',R') \subseteq V\) with majorant \(h'\), a point inside
\(B(c',R')\) either lies outside \(B(c,R)\) — where the modification is
\(v\) and \(v \le h'\) follows from \(v\)'s own clause via
\(v \le \texttt{poissonSolution}\,v\,c\,R \le h'\) on the circle — or lies
in the overlap \(U = B(c',R') \cap B(c,R)\), where the modification is the
harmonic Poisson operator; there \(w = \texttt{poissonSolution} - h'\) is
harmonic on \(U\), continuous on \(\overline{U}\), and \(\le 0\) on
\(\operatorname{frontier} U\) (each frontier point lies on
\(\partial B(c',R')\), where \(h'\) dominates by hypothesis, or on
\(\partial B(c,R)\), where the modification is \(v \le h'\)).  This overlap
clause is the **only** consumer of the weak maximum principle
\(\texttt{WeakMaxPrincipleInput}\) (W1, owner lane jc3); the comparison
half and continuity are obligation-free.  This is the W5b-iii membership
fact the §3.5 Perron envelope consumes (the envelope sup is built by
Poisson-modifying members on chart discs).  Sorry-free now; the W1
hypothesis discharges once W1 lands.
\lean{JacobianChallenge.HolomorphicForms.PerronSubOn.le_poissonSolution_of_mem_ball, JacobianChallenge.HolomorphicForms.PerronSubOn.poissonSolution_continuousOn, JacobianChallenge.HolomorphicForms.PerronSubOn.poissonModify, JacobianChallenge.HolomorphicForms.harmonicContOnCl_poissonSolution}
\leanok
\end{lemma}

\begin{lemma}[W6 log cup (green leaf)]
\label{lem:w6-log-cup}
The logarithmic comparison cup
\(\texttt{logCup}\,c\,r\,\varepsilon = \varepsilon(\log r - \log\lVert\cdot - c\rVert)\)
of the W6 removable-singularity development: harmonic off the puncture
(from Mathlib's \texttt{AnalyticAt.harmonicAt\_log\_norm}), vanishing on
the outer circle, nonnegative on the punctured closed disc, continuous
off the puncture, and climbing past every constant near the puncture.
Cited as an already-green project leaf: at weight \(1\) the cup is the
shifted log profile \(-\ell_p + \log\rho\) from which the W9 Green's
family builds its explicit member.
\lean{JacobianChallenge.HolomorphicForms.logCup, JacobianChallenge.HolomorphicForms.logCup_harmonicOnNhd, JacobianChallenge.HolomorphicForms.logCup_eq_zero_of_mem_sphere, JacobianChallenge.HolomorphicForms.logCup_nonneg, JacobianChallenge.HolomorphicForms.logCup_continuousOn, JacobianChallenge.HolomorphicForms.eventually_le_logCup}
\leanok
\end{lemma}

\begin{lemma}[Bounded-harmonic removable singularity (W6)]
\label{lem:perron-removable-singularity}
\uses{lem:w1-weak-max-principle, lem:m4-poisson-solution, lem:w6-log-cup}
A function \(u\) harmonic on the punctured ball
\(B(c,R) \setminus \{c\}\) and bounded there extends harmonically across
the puncture: there is \(v\) harmonic on all of \(B(c,R)\) agreeing with
\(u\) off the puncture; consequently \(u\) has a limit at the puncture —
the extension's value.  The engine is the annuli comparison: on an inner
disc \(\overline{B}(c,r)\), \(r < R\), the function \(u\) agrees off the
puncture with every disc-Dirichlet solution \(h\) of its inner-circle
restriction, by comparing \(\pm(u - h)\) against the log cup
\(\varepsilon(\log r - \log\lVert\cdot - c\rVert)\) on shrinking annuli
via the weak maximum principle and letting \(\varepsilon \to 0\); the
extension is then the piecewise glue (\(h\) inside, \(u\) outside),
harmonic pointwise by germ equality with one of its two branches.  The
weak maximum principle is jc3's
\(\texttt{weakMaxPrincipleInput\_holds}\)
(\ref{lem:w1-weak-max-principle}); the Dirichlet solution is jc6's M4
packaging consumed through the adapter
\(\texttt{discDirichletInput\_of\_poissonOperator}\)
(\ref{lem:m4-poisson-solution}).  The primed theorems are the
hypothesis-free forms; the parametric originals are retained for
consumers that thread obligation terms.  This is the B2 toolbox row W6,
consumed by the W9 Green's-function pole germ (§3.3).
\lean{JacobianChallenge.HolomorphicForms.discDirichletInput_of_poissonOperator, JacobianChallenge.HolomorphicForms.eq_dirichletSolution_of_bounded_punctured, JacobianChallenge.HolomorphicForms.eq_dirichletSolution_of_bounded_punctured', JacobianChallenge.HolomorphicForms.harmonicOnNhd_extend_of_bounded_punctured, JacobianChallenge.HolomorphicForms.harmonicOnNhd_extend_of_bounded_punctured', JacobianChallenge.HolomorphicForms.exists_tendsto_of_bounded_punctured, JacobianChallenge.HolomorphicForms.exists_tendsto_of_bounded_punctured'}
\leanok
\end{lemma}
\begin{proof}\leanok
For the annuli comparison: \(w = \pm(u - h)\) is harmonic on the
punctured inner ball, continuous up to its closure off the puncture,
\(\le 0\) on the inner circle, and bounded; on the annulus
\(\{\rho < \lVert z - c\rVert < r\}\) the cup
\(\varepsilon(\log r - \log\lVert\cdot - c\rVert)\) dominates \(w\) on
both frontier circles once \(\rho\) is small (the cup climbs past every
constant near the puncture), so the weak maximum principle
(\ref{lem:w1-weak-max-principle}) forces
\(w \le \varepsilon\,\texttt{logCup}\) inside; \(\varepsilon \to 0\)
gives \(w \le 0\), and the two signs give equality.  For the extension:
glue \(h\) (from \ref{lem:m4-poisson-solution} through the adapter)
inside the half-radius disc with \(u\) outside; the comparison makes the
branches agree on the overlap off the puncture, and harmonicity at each
point is germ equality with the harmonic branch.  The limit corollary
evaluates the extension at the puncture and transports the limit along
agreement off the puncture.
\end{proof}

\begin{definition}[Per-pole Perron family and Green's candidate]
\label{def:perron-greens-family}
\uses{def:perron-sub-on, lem:perron-envelope-center-touch, lem:w6-log-cup}
For an open stage-local domain \(V \subseteq \mathbb{C}\) and pole
\(p\), the Green's family \(\mathfrak{F}_p := \texttt{greensFamily}\,V\,p\)
(pricing doc §3.3) consists of the \(\texttt{PerronSubOn}(V \setminus \{p\})\)
members that are eventually \(\le \varepsilon\) at every frontier point
of \(V\) (the boundary trap, in limsup-free eventual form, which closes
under \(\max\) by filter intersection) and whose shift
\(v + \log\lVert\cdot - p\rVert\) is bounded above near \(p\) (the
member-dependent pole cap).  The explicit member
\(\texttt{truncatedLogCap}\,p\,\rho = \max(\texttt{logCup}\,p\,\rho\,1, 0)\)
is §3.3's truncated log cap — the \(\max\) is the seam gluing.  The
Green's-function candidate is the frozen Perron envelope of the family:
\(\texttt{greensCandidate}\,V\,p := \texttt{perronEnvelope}\,(\texttt{greensFamily}\,V\,p)\).
\lean{JacobianChallenge.HolomorphicForms.greensFamily, JacobianChallenge.HolomorphicForms.truncatedLogCap, JacobianChallenge.HolomorphicForms.greensCandidate}
\leanok
\end{definition}

\begin{lemma}[Green's family: closure, members, pole lower profile]
\label{lem:perron-greens-family-basics}
\uses{def:perron-greens-family, lem:perron-sub-on-harmonic-member, lem:w1-weak-max-principle, lem:w6-log-cup}
The W9 G1 basics, sorry-free.  The family is closed under pointwise
\(\max\) (the W7 \texttt{hmax} field verbatim) and nonempty: the zero
function is a member with no geometric hypotheses (harmonic, hence a
member by the W1-discharged harmonic-membership; trapped at the
boundary; capped at the pole with \(C = 0\)).  For a comparison disc
\(\overline{B}(p, \rho) \subseteq V\) the truncated log cap is a member
— the \(\max\) of two harmonic members for the comparison clause, zero
near the frontier for the trap, and with the \emph{exact} pole constant
\(\log\rho\): inside the punctured disc
\(\texttt{truncatedLogCap} + \log\lVert\cdot - p\rVert = \log\rho\).
Consequently the envelope is nonnegative on \(V \setminus \{p\}\) and
satisfies the pole \emph{lower} profile
\(\log\rho \le \texttt{greensCandidate}\,V\,p + \log\lVert\cdot - p\rVert\)
eventually at \(p\) (both modulo the pointwise boundedness supplied by
the G2 cap).
\lean{JacobianChallenge.HolomorphicForms.greensFamily_max_mem, JacobianChallenge.HolomorphicForms.zero_mem_greensFamily, JacobianChallenge.HolomorphicForms.greensFamily_nonempty, JacobianChallenge.HolomorphicForms.truncatedLogCap_mem_greensFamily, JacobianChallenge.HolomorphicForms.logCup_one_apply, JacobianChallenge.HolomorphicForms.greensCandidate_nonneg, JacobianChallenge.HolomorphicForms.log_le_greensCandidate_add_log}
\leanok
\end{lemma}

\begin{lemma}[Comparison-subclass maximum principle, eventual-boundary form]
\label{lem:perron-sub-on-max-principle}
\uses{def:perron-sub-on, lem:perron-sub-on-poisson-modify, lem:m4-poisson-solution, lem:w1-weak-max-principle}
A \(\texttt{PerronSubOn}\) member of a bounded open \(W \subseteq \mathbb{C}\)
that is eventually \(\le \varepsilon\) at every frontier point (for
every \(\varepsilon > 0\)) is nonpositive throughout \(W\) — the W9 G2
provider.  The landed W1 form does not apply (members are continuous on
\(W\) only, and the boundary control is eventual), but its M2 component
skeleton transfers: the harmonic mean-value property is replaced by the
\emph{Poisson sub-mean inequality} for members
(\(v(x) \le \texttt{circleAverage}\,v\,x\,r\), from the W5b-iii
comparison half evaluated at the center, where the Poisson kernel is
constantly \(1\)), the W1 averaging rigidity upgrades the squeeze at an
interior maximum to constancy on concentric circles, and the bounded
connected component of the maximum exits at a frontier point whose trap
bounds the maximum by \(\varepsilon\) on a \texttt{NeBot}
within-filter.  Helpers: the center normalization
\(\texttt{poissonOperator\_center}\), the member local-max propagation,
and \(\operatorname{frontier}(V \setminus \{p\}) =
\operatorname{frontier} V \cup \{p\}\).
\lean{JacobianChallenge.HolomorphicForms.poissonOperator_center, JacobianChallenge.HolomorphicForms.PerronSubOn.le_circleAverage, JacobianChallenge.HolomorphicForms.PerronSubOn.eq_const_closedBall_of_le, JacobianChallenge.HolomorphicForms.PerronSubOn.le_zero_of_eventually_frontier_le}
\leanok
\end{lemma}

\begin{lemma}[Green's candidate: uniform cap and two-sided pole profile]
\label{lem:perron-greens-candidate-cap-profile}
\uses{def:perron-greens-family, lem:perron-greens-family-basics, lem:perron-sub-on-max-principle, lem:perron-sub-on-add-harmonic, lem:w6-log-cup}
The W9 G2 payload.  \emph{Uniform upper cap}: every member satisfies
\(v \le \texttt{truncatedLogCap}\,p\,\rho + C\) on \(V \setminus \{p\}\)
with the member-independent constant \(C = \log R_0 - \log\rho\), where
\(V \subseteq B(p, R_0)\).  The member-dependent pole constant washes
out by a \(\theta \to 0\) device with no annuli: for \(\theta > 0\) the
harmonic shift \(v + \texttt{logCup}\,p\,R_0\,(-(1+\theta))\) is again
a member (W5b closure under harmonic shifts), near the pole the excess
\(\theta\log\lVert\cdot - p\rVert \to -\infty\) swallows the member's
cap, so the eventual-boundary maximum principle applies on all of
\(V \setminus \{p\}\); letting \(\theta \to 0\) leaves
\(v \le \log R_0 - \log\lVert\cdot - p\rVert\).  Pointwise boundedness
of the family follows (\texttt{greensFamily\_bddAbove}).
\emph{Two-sided pole profile}:
\(|\texttt{greensCandidate}\,V\,p + \log\lVert\cdot - p\rVert| \le C'\)
eventually at \(p\) — the lower half is the G1 basics (the explicit
member), the upper half is the cap through
\(\texttt{perronEnvelope\_le}\).
\lean{JacobianChallenge.HolomorphicForms.greensFamily_le_truncatedLogCap_add, JacobianChallenge.HolomorphicForms.greensFamily_bddAbove, JacobianChallenge.HolomorphicForms.greensCandidate_pole_profile}
\leanok
\end{lemma}

\begin{lemma}[Green's candidate: modification, harmonicity, germ (open)]
\label{lem:perron-greens-candidate-pole-profile}
\uses{def:perron-greens-family, lem:perron-greens-family-basics, lem:perron-greens-candidate-cap-profile, lem:perron-sub-on-poisson-modify, lem:m4-poisson-solution, lem:harmonic-disc-comparison, lem:perron-envelope-center-touch, lem:w1-weak-max-principle}
The remaining W9 frontier statements (slices G3–G4).  \emph{Poisson
modification} (G3): on a disc \(\overline{B}(c, R) \subseteq V \setminus \{p\}\)
the M4 modification of a member is a dominating member — the comparison
clause is the landed W5b-iii membership, the boundary and pole clauses
ride the seam (the modification equals the member off the compact
modification disc).  \emph{Off-pole harmonicity} (G3): conditional on
the W7-ii obligation \(\texttt{PerronEnvelopeHarmonicInput}\) (W7-i's
exact hypothesis list, conclusion upgraded to harmonic-throughout;
jc11's lane discharges it by thin adapter), instantiated on comparison
discs inside the punctured domain.  \emph{W6 germ} (G4): the shifted
candidate is harmonic and bounded on a punctured disc, so the
removable-singularity surface
(\texttt{exists\_tendsto\_of\_bounded\_punctured}, with the discharged
W1/W3 inputs) gives the limit at the pole — the germ datum the W10
stage assembly consumes at \(P_0\)/\(P_\infty\).
\lean{JacobianChallenge.HolomorphicForms.greensFamily_poissonModificationInput, JacobianChallenge.HolomorphicForms.PerronEnvelopeHarmonicInput, JacobianChallenge.HolomorphicForms.greensCandidate_harmonicOnNhd_off_pole, JacobianChallenge.HolomorphicForms.greensCandidate_add_log_tendsto}
\end{lemma}

\begin{lemma}[Stage chart-reading uniform Cauchy bounds (open)]
\label{lem:stage-chart-reading-uniform-cauchy-bound}
\uses{lem:stage-map-normalized-noncollapse, lem:stage-dipole-boundary-control, lem:stage-eventual-containment-bounds}
...
normalization.
\lean{JacobianChallenge.HolomorphicForms.stageChartReading, JacobianChallenge.HolomorphicForms.StageChartReadingUniformCauchyBound, JacobianChallenge.HolomorphicForms.stageChartReading_uniform_cauchy_bound}
\end{lemma}

\begin{lemma}[Stage chart readings as ChartBallPowerSeries (open)]
\label{lem:stage-chart-reading-chartBallPowerSeries}
\uses{lem:stage-cut-domain-conjugate-control, lem:stage-holomorphic-coordinate-from-harmonic-dipole, lem:stage-chart-reading-uniform-cauchy-bound, lem:montel-stage-compact-target-package}
...
\lean{JacobianChallenge.HolomorphicForms.stageChartReading_chartBallPowerSeries}
\end{lemma}

\begin{lemma}[Stage common diagonal subsequence]\lean{JacobianChallenge.HolomorphicForms.exists_stage_common_diagonal_subsequence}\leanok
\label{lem:stage-common-diagonal-subsequence}
\uses{lem:stage-chart-reading-uniform-cauchy-bound, lem:stage-chart-reading-chartBallPowerSeries, lem:montel-finite-family-locally-uniform-diagonal-extraction}
...
\begin{lemma}[Stage engine F-clause transfer]\lean{JacobianChallenge.HolomorphicForms.tendstoLocallyUniformlyOn_F_clause}\leanok
\label{lem:stage-engine-f-clause-transfer}
\uses{lem:montel-realized-patch-coord-convergence-transfer, lem:stage-common-diagonal-subsequence}
The selection target's F-clause is producible from spine outputs by green
composition. Given the engine's total family and per-index chart-ball-domain
convergence (the diagonal subsequence output shape) along with per-index
agreement on each source, we produce the full selection-target F-clause
locally-uniform convergence of the patch coordinate.
\end{lemma}

\begin{lemma}[Stage total filler locally uniform irrelevance]
...
facts are available.
\lean{JacobianChallenge.HolomorphicForms.stageTotalMapWithFiller, JacobianChallenge.HolomorphicForms.StageTotalMaps, JacobianChallenge.HolomorphicForms.exists_stageTotalMaps, JacobianChallenge.HolomorphicForms.stageTotalMapWithFiller_eventually_eqOn_of_eventually_subset_cutDomain, JacobianChallenge.HolomorphicForms.tendstoUniformlyOn_congr_eventuallyEqOn, JacobianChallenge.HolomorphicForms.tendstoLocallyUniformlyOn_congr_eventuallyEqOn, JacobianChallenge.HolomorphicForms.tendstoUniformlyOn_stageTotalMapWithFiller_iff, JacobianChallenge.HolomorphicForms.tendstoLocallyUniformlyOn_stageTotalMapWithFiller_iff, JacobianChallenge.HolomorphicForms.StageTotalMaps.eventually_eqOn_of_eventually_subset_cutDomain, JacobianChallenge.HolomorphicForms.StageTotalMaps.tendstoUniformlyOn_iff_of_eventually_subset_cutDomain, JacobianChallenge.HolomorphicForms.StageTotalMaps.tendstoLocallyUniformlyOn_iff_of_eventually_subset_cutDomain}
\leanok
\end{lemma}

\begin{lemma}[Stage engine common sequence payload (open)]
\label{lem:stage-engine-common-sequence-payload}
\uses{lem:stage-common-diagonal-subsequence, lem:stage-total-filler-locally-uniform-irrelevance, lem:stage-map-normalized-noncollapse, lem:stage-engine-f-clause-transfer}
...
\uses{lem:montel-selector-finite-patch-family-assembly, lem:montel-selector-patch-family-extraction, lem:montel-selector-source-patch-cover, lem:global-gluing-coord-mem-target, lem:montel-selector-chart-inverse-local-value, lem:montel-selector-normal-limit-agrees-with-uniformization, lem:montel-selector-overlap-compatible-normalized-values}
...
divisor-compatible addition fold.
