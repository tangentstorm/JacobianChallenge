...
chart-local order one, stated carrier-side) as a hypothesis. Membership
...
order-one field, which is the hypothesis. The side condition is provably not
derivable from current carrier fields (see the CARRIER-GATED
Lemma~\ref{lem:germ-order-bridge}); the statement-level hypotheses here are
instantiated definitionally by the carrier wrapper once the upstream
duplicate-declaration reconciliation and carrier order-soundness land, at
which point the open germ-to-map bridge
(Lemma~\ref{lem:rr-germ-to-map-bridge}) reduces to producing the
representative.
...
CARRIER-GATED: unprovable from the current \code{MeromorphicFunctionWithDivisors} fields (counter-model: recorded order 0 against a genuine root of order 3 satisfies all fields); requires an order--\code{orderAt} compatibility field in the carrier (jc0's lane).
\end{lemma}
