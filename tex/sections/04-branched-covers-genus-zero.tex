...
\(\texttt{PerronSubOn}(V)\) member, given the harmonic weak maximum
principle as an input.  On a disc \(\overline{B}(c,R) \subseteq V\) with a
harmonic-up-to-closure majorant \(h'\) dominating \(h\) on
...
members).  The weak maximum principle is taken as the explicit hypothesis
\(\texttt{WeakMaxPrincipleInput}\) (the W1 obligation, owner lane jc10);
the lemma is sorry-free now and the hypothesis discharges trivially once
W1 is proved.
\lean{JacobianChallenge.HolomorphicForms.PerronSubOn.of_harmonicOnNhd}
\leanok
\end{lemma}
...
\(h_1 \le h_2\) throughout \(B(c,R)\) — given the harmonic weak maximum
principle as an input.  The difference \(w = h_1 - h_2\) is harmonic on
\(B(c,R)\), continuous on \(\overline{B}(c,R)\), and \(\le 0\) on
\(\partial B(c,R) = \operatorname{frontier} B(c,R)\); the weak maximum
...
to \ref{lem:perron-sub-on-harmonic-member}.  The weak maximum principle
is taken as the explicit hypothesis \(\texttt{WeakMaxPrincipleInput}\)
(W1, owner lane jc10); sorry-free now, discharging once W1 is proved.
\lean{JacobianChallenge.HolomorphicForms.harmonicContOnCl_le_of_le_on_sphere}
\leanok
\end{lemma}
