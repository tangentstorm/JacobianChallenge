...
\(\code{OnePoint}\,\C\) on the target side. This is the real
\code{target\_cover\_two\_chart} field of
\code{GenusZeroTwoChartMontelCoverSelector}; wiring it separately makes the
target-cover obligation visible in the selector spine.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroTwoChartMontelCoverSelector.target_cover_two_chart}
\end{lemma}
...
fields are real; the missing provider is the construction that packages the
two-chart normalized-limit selector as this global family.
\lean{JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily, JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily.patch_cover, JacobianChallenge.HolomorphicForms.GenusZeroGlobalPatchFamily.target_chart_cover}
\end{lemma}
