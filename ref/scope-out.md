...
| `def:branched-degree` | `Blueprint.branchedDegree` | `Sec02/BranchedDegree.lean` | SHORT | leaves 1–7 sorry-free in `Sec02/BranchedDegree.lean`; leaf 8 split out to `Sec02/BranchedDegreeFromHolomorphic.lean` with a meaningful signature (`IsHolomorphic f` + nonconstant) but `sorry` body — blocked on chart-independence of `mapAnalyticOrderAt`, finite-fibre identity-principle argument, and constancy of weighted fibre count (≈degree well-definedness) |
| (project-local) `IsHolomorphic` predicate | `HolomorphicMap.IsHolomorphic` / `IsHolomorphicAt` / `mapAnalyticOrderAt` | `HolomorphicForms/HolomorphicMap.lean` | DONE | sorry-free scaffold for "holomorphic map between two `ChartedSpace ℂ`-equipped spaces" — bridges Mathlib's `AnalyticAt ℂ` to the manifold-level analytic predicate that Mathlib v4.28.0 lacks |
