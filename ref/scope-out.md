...
| `def:meromorphic-function` | `Blueprint.MeromorphicFunctionType` | `Sec01/MeromorphicFunction.lean` | DECOMPOSE | sub-leaves: meromorphic-germ-sheaf, field-of-fractions on Riemann surface; placeholder now also carries CP¹ holomorphicity, order/value comparison, local mapping, and weighted-fibre packages |
...
| `thm:principal-degree-zero` | `Blueprint.principal_degree_zero` | `Sec01/PrincipalDegreeZero.lean` | DECOMPOSE | umbrella body sorry-free (CP¹ branched-cover route, `ref/plans/principal-degree-zero.md` + `principal-degree-zero-breakthrough.md`); leaves 1, 2, 3, 4, 5, 5a, 6, 7a, 7b, 7c, `liftToCp1_continuous`, umbrella all discharged through Phase A–D/R4–R17 refactors; no remaining transitive Lean sorries in this route; remaining mathematics sits at `def:meromorphic-function` placeholder API; promoted to `\leanok` per integrator convention |
...
| `lem:montel-compactness` | `Blueprint.montel_compactness` | `Sec02/MontelCompactness.lean` | DONE | Worker M landed; umbrella body sorry-free assembly above explicit frontier axiom `montel_pointwise_extraction` |
...
| `thm:fd-holomorphic-one-forms` | `Blueprint.fd_holomorphic_one_forms` | `Sec02/FdHolomorphicOneForms.lean` | DONE | sorry-free via production Banach/local-compactness assembly and `FiniteDimensional.of_locallyCompactSpace` |
| `def:sheaf-cohomology-rs` | `<missing>` | `<missing>` | DECOMPOSE | sub-leaves: sheaf, sheafification, Čech cohomology, derived functors on RS |
| `thm:serre-duality-rs` | `serre_duality_rs` | `Jacobian/HolomorphicForms/SerreDualityRS.lean` (+ `Serre/*.lean`) | DECOMPOSE (in flight) | 25 top-down rounds landed: tree under `Jacobian/HolomorphicForms/Serre/` (32 files) decomposes the public `serre_duality_rs` and `RSDualizingSheaf` sorries into ~80 strictly-smaller named obligations covering: structure/cotangent/tensor sheaves (presheaf + sheaf-condition splits), canonical Serre dual `Hom(F, K_X)`, cup-product/eval-map/trace-map decomposition of the pairing, witness-form nondegeneracy via Dolbeault + L²-pairing on harmonic forms, residue-map model of `H¹(K_X) ≃ ℂ` now isolated as `residueMapIso`, line-bundle specialisation (`L⁻¹ ⊗ K_X`), Riemann-Roch high-degree reduction via Serre, `h1_dualizing_sheaf_one_dim` discharged sorry-free via the canonical iso, finite-dim instance discharges for `K_X` / `𝒪_X`, identifications `H⁰(K_X) ≃ HolomorphicOneForm X` and `H¹(𝒪_X) ≃ HolomorphicOneForm X *`. Plan: `ref/plans/serre-duality-rs.md`. |
...
| `input:riemann-roch` | `Blueprint.input_riemann_roch` / `Blueprint.riemann_roch_umbrella_exists` | `Sec02/InputRiemannRoch.lean` | DECOMPOSE | identity theorem is sorry-free; umbrella existence is an explicit frontier axiom pending euler-char-line-bundle + serre-duality-rs |
...
| `input:degree-one-isomorphism` | `Blueprint.input_degree_one_isomorphism` / `Blueprint.degree_one_isomorphism` | `Sec02/InputDegreeOneIsomorphism.lean`, `Sec02/DegreeOneIsomorphism.lean` | DONE | topological compact-to-T2 homeomorphism umbrella is sorry-free via `degree_one_bijective`; analytic-isomorphism scaffold remains a placeholder statement |

## sec03 — Periods and Riemann bilinear

| Label | Decl | File | Class | Sketch |
|---|---|---|---|---|
| `lem:period-homology-invariance` | typed form: `Periods.period_homology_invariance_statement` (sorry-free `congrArg` in `WorkPackets/StatementBank.lean`) + `Blueprint.Sec03.period_homology_invariance`; descent obligation: `Blueprint.Sec03.period_homology_invariance_descent` | `Jacobian/WorkPackets/StatementBank.lean`, `Jacobian/Blueprint/Sec03/PeriodHomologyInvariance.lean`, `Jacobian/Blueprint/Sec03/HolomorphicFormIsClosed.lean`, `Jacobian/Blueprint/Sec03/StokesOnRSWithBoundary.lean` | DECOMPOSE | typed/descent assemblies are sorry-free; remaining chain-boundary vanishing is isolated as explicit frontier axiom `chain_integration_kills_boundary` |
...
| `def:symplectic-basis` | `Blueprint.Sec03.symplecticBasis` | `Sec03/SymplecticBasis.lean` | DECOMPOSE | standard form and assembly are sorry-free; H₁ rank, intersection nondegeneracy, and basis existence are explicit frontier axioms |
...
| `lem:principal-deg0-simple-support-deg1` | `Blueprint.principal_deg0_simple_support_deg1` + nested arithmetic helper | `Sec05/PrincipalDeg0SimpleSupportDeg1.lean` | DECOMPOSE | headline sorry-free above explicit branched-degree analytic constructor axiom; zero/pole point-value leaves and nested two-point degree-zero arithmetic are sorry-free |
| `thm:riemann-hurwitz-deg1` | `Blueprint.AbelExistence.RiemannHurwitzDeg1.riemann_hurwitz_deg1` | `Sec05/RiemannHurwitzDeg1.lean` | DONE | sorry-free assembly; `SurfaceMap` placeholder carries Riemann-Hurwitz and degree-one no-ramification fields; Euler-characteristic route in `RiemannHurwitzViaEulerChar.lean` is also sorry-free relative to the same placeholder |
