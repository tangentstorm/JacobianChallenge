...
| Label | Decl | File | Class | Sketch |
|---|---|---|---|---|
| `lem:period-homology-invariance` | typed form: `Periods.period_homology_invariance_statement` (sorry-free `congrArg` in `WorkPackets/StatementBank.lean`) + `Blueprint.Sec03.period_homology_invariance` (sorry-free `congrArg` in `Blueprint/Sec03/PeriodHomologyInvariance.lean`); descent obligation: `Blueprint.Sec03.period_homology_invariance_descent` (sorry-free assembly delegating to sub-leaf D); 4 named sub-leaves: `exists_singularChain_integration` (A, sorry — chain-level integration of holomorphic forms), `holomorphicForm_closed_chain_integral` (B, forwards to existing `holomorphic_form_is_closed` stub), `stokes_chain_integral_boundary` (C, `True` placeholder for `∫_{∂s} η = ∫_s dη`), `chainIntegral_kills_boundary_of_closed` (D, sorry — assembly target for "I (∂₂ s) η = 0") | `Jacobian/WorkPackets/StatementBank.lean`, `Jacobian/Blueprint/Sec03/PeriodHomologyInvariance.lean` | DECOMPOSE | typed form sorry-free; descent obligation refined into sorry-free assembly + 4 Aristotle-shaped sub-leaves (A, D carry sorries; B, C delegate to existing Sec03 stubs `holomorphic_form_is_closed` and `stokes_on_rs_with_boundary`) |
