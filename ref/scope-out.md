...
| `lem:weighted-fiber-locally-constant` (new) | `Blueprint.isHolomorphic_weightedFiberSum_isLocallyConstant` | `Sec02/WeightedFiberCardConst.lean` | HARD | sub-leaf D, `sorry`. Combines local-inj + local-kfold + finite-fibre + T2 disjoint-nbhd; proof sketch in file docstring. |
| `lem:local-kfold-ramified` (new) | `Blueprint.IsHolomorphicAt.exists_local_kfold_of_ramified` | `Sec02/WeightedFiberCardConst.lean` | HARD | sub-leaf C, `sorry`. `f(t) = t^k · g(t)` form via `analyticOrderAt_eq_natCast` + holomorphic `k`-th root. |
| `lem:local-inj-unramified` (new) | `Blueprint.IsHolomorphicAt.exists_local_inj_of_unramified` | `Sec02/WeightedFiberCardConst.lean` | MEDIUM | sub-leaf B, `sorry`. Mathlib's `AnalyticAt.localInverse` + chart transport. |
| `mapAnalyticOrderAt_ramified_finite` (new) | `Blueprint.mapAnalyticOrderAt_ramified_finite` | `Sec02/WeightedFiberCardConst.lean` | MEDIUM | sub-leaf A, `sorry`. Discrete zero set of analytic derivative + compactness ⇒ finite. |
