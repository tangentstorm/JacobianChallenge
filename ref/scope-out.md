...
| `def:branched-degree` | `Blueprint.branchedDegree` | `Sec02/BranchedDegree.lean` | SHORT | leaves 1–7 sorry-free in `Sec02/BranchedDegree.lean`; leaf 8 in `Sec02/BranchedDegreeFromHolomorphic.lean` discharges 3 of 4 obligations (ramificationIndex/positivity/finite_fiber) — single remaining sorry is `weightedFiberCard_const` (well-definedness of degree, the genuinely hard analytic gap) |
