...
Trace/degree/push-pull           ████████████░░░░░░░░   59%  pullbackFormsFun: full linearity + LinearMap bundle + id + comp-id/comp-const + const-of-const + mixed const/id + id-of-id + light bridge to HolomorphicForms.evalLinearMap + bundled along-id+along-const full dist + bundled along-(id ∘ id) full forwarder bank + bundled comp-const/const-comp full form/vec-slot dist + bundled `pullbackFormsLinearMap` ℕ/ℤ-smul + zero/neg/sub identities for arbitrary f + smul/neg/add combined identities + ℕ/ℤ-smul × add/sub (function + vec-applied)
