...
Last tick: 2026-04-27 10:43 EDT
...
Trace/degree/push-pull           █████████░░░░░░░░░░░   45%  pullbackFormsFun: full linearity + LinearMap bundle + id + comp-id/comp-const + const-of-const + mixed const/id + id-of-id + light bridge to HolomorphicForms.evalLinearMap + bundled along-id+along-const full dist + bundled along-(id ∘ id) full forwarder bank + bundled comp-const/const-comp full form/vec-slot dist (add/neg/sub/ℂ-smul/ℕ-smul both sides)
...
Integrated this tick (local Claude-owned, 4 lemmas):
                      NEW TraceDegree.PullbackFormsLinearMapCompConstIntSmul:
                      bundled comp-const/const-comp ℕ-smul
                      (form-slot and vec-slot both sides).
Note:                 Untracked LICENSE file in working tree —
                      unrelated, not committed this tick.
...
TraceDegree.PullbackFormsLinearMapCompConstSmul pass lake build Jacobian.TraceDegree.PullbackFormsLinearMapCompConstSmul (no sorry)
TraceDegree.PullbackFormsLinearMapCompConstVec pass lake build Jacobian.TraceDegree.PullbackFormsLinearMapCompConstVec (no sorry)
TraceDegree.PullbackFormsLinearMapCompConstVecExtra pass lake build Jacobian.TraceDegree.PullbackFormsLinearMapCompConstVecExtra (no sorry)
TraceDegree.PullbackFormsLinearMapCompConstIntSmul pass lake build Jacobian.TraceDegree.PullbackFormsLinearMapCompConstIntSmul (no sorry)
