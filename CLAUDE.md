...
- **Agent Status & Unblocking**: All workers must manage their active checklists in `task.md` using the standardized status lines (`STATUS: WORKING` | `STATUS: STEP-DONE` | `STATUS: BLOCKED`) and follow the strict local-infrastructure unblocking protocol defined in [ref/status-guide.md](file:///Users/michal/ver/jc0/ref/status-guide.md). Never declare yourself blocked by Mathlib gaps—build the missing helper lemmas locally!
