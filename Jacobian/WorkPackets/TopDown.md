...
| `genus_eq_zero_iff_homeo` ✅ refined | `analyticGenus_eq_zero_iff_homeomorphic_sphere` (assembly, no own sorry) | `analyticGenus_eq_zero_of_homeomorphic_sphere` (mp) is split through the `OnePoint ℂ` holomorphic-form computation. `homeomorphic_sphere_of_analyticGenus_eq_zero` (mpr) now delegates to `genus_zero_homeomorph_onePointCx` plus the proved `onePointCx_homeomorph_sphere`; `genus_zero_homeomorph_onePointCx` is further split into `genus_zero_exists_simplePole_meromorphicMap` (Riemann-Roch simple-pole existence), `simplePole_meromorphicMap_proper_degreeOne` (properness/degree-one promotion), and `proper_degreeOne_meromorphicMap_biholomorphic` (discharged topological assembly from compact-to-T2). |
