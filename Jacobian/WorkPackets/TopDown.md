...
| `genus_eq_zero_iff_homeo` ✅ refined | `analyticGenus_eq_zero_iff_homeomorphic_sphere` (assembly, no own sorry) | `analyticGenus_eq_zero_of_homeomorphic_sphere` (mp) is split through the `OnePoint ℂ` holomorphic-form computation; `holomorphicOneForm_coeff_entire` is now a sorry-free wrapper around opaque chart-extraction data `holomorphicOneFormCoeffEntireData`, and the homeomorphic-sphere vanishing leaf is now a sorry-free wrapper around opaque data `homeoSphereHolomorphicOneFormVanishing`. `homeomorphic_sphere_of_analyticGenus_eq_zero` (mpr) now delegates to `genus_zero_homeomorph_onePointCx` plus the proved `onePointCx_homeomorph_sphere`; `genus_zero_homeomorph_onePointCx` is further split into sorry-free wrappers around opaque data `simplePoleMeromorphicMapOfGenusZero` (Riemann-Roch simple-pole existence) and `properDegreeOneMapOfSimplePole` (properness/degree-one promotion), followed by `proper_degreeOne_meromorphicMap_biholomorphic` (discharged topological assembly from compact-to-T2). |
