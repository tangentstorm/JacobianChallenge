...
| `genus_eq_zero_iff_homeo` ✅ refined | `analyticGenus_eq_zero_iff_homeomorphic_sphere` (assembly, no own sorry) | `analyticGenus_eq_zero_of_homeomorphic_sphere` (mp) is split through the `OnePoint ℂ` holomorphic-form computation; `holomorphicOneForm_coeff_entire` now assembles from identity-chart leaves `holomorphicOneFormIdentityChartCoeffContDiff` and `holomorphicOneForm_coeff_eq_identityChartCoeff`; `holomorphicOneForm_coeff_tendsto_zero` now assembles from inversion-chart leaves `holomorphicOneFormInversionChartCoeffContinuousAtZero`, `holomorphicOneForm_inversionCoeff_eq_inversionChartCoeff`, `holomorphicOneForm_identityInversionTransition_eventually`, and `holomorphicOneFormCoeffTendstoZeroOfTransition`; `holomorphicOneForm_onePointCx_toFun_infty_eq_zero` now uses the real punctured-chart lemma `holomorphicOneForm_inversionCoeff_eq_zero_of_ne_zero` plus theorem leaf `holomorphicOneForm_infty_vanishing_of_inversionCoeff`; the homeomorphic-sphere vanishing leaf is now theorem leaf `homeoSphereHolomorphicOneFormVanishing`. `homeomorphic_sphere_of_analyticGenus_eq_zero` (mpr) delegates to `genus_zero_homeomorph_onePointCx` plus the proved `onePointCx_homeomorph_sphere`; `genus_zero_homeomorph_onePointCx` is split into a real packaging definition `simplePoleMeromorphicMapOfGenusZero` over fixed-pole leaves `genusZeroRiemannRochNonconstantMapAt` (Riemann-Roch map at a prescribed point) and `genusZeroRiemannRochSimplePoleAt` (divisor/order certificate), plus `properDegreeOneMapOfSimplePole_nonempty` feeding data definition `properDegreeOneMapOfSimplePole`, followed by `proper_degreeOne_meromorphicMap_biholomorphic` (discharged topological assembly from compact-to-T2). |
