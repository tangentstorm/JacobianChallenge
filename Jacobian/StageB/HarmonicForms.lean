...
-- Real definition: `LinearMap.ker (laplaceBeltrami M n k)`; but the
-- elaboration depends on the deferred Riemannian-metric data.
...
theorem laplaceBeltrami_elliptic_regularity
    ...
    Module.Finite ℝ (Harmonic (E := E) M n k) := sorry
...
theorem deRhamH_iso_Harmonic [CompactSpace M] (n k : ℕ) :
    Nonempty (deRhamH (E := E) M k ≃ₗ[ℝ] Harmonic (E := E) M n k) := sorry
...
instance compact_deRhamH_finite [CompactSpace M] (n k : ℕ) :
    FiniteDimensional ℝ (deRhamH (E := E) M k) := sorry
...
theorem elliptic_compact_finite_kernel (n k : ℕ) : True := sorry
