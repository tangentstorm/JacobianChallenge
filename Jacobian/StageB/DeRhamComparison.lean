...
noncomputable def deRhamMap (k : ℕ) :
    deRhamH (E := E) M k →ₗ[ℝ] (Module.Dual ℝ ℝ) := sorry
...
theorem deRhamMap_well_defined (k : ℕ) : True := sorry
...
theorem simplex_pullback_form (_k : ℕ) : True := sorry

/-- **Round 1.** *Sub-leaf:* Lebesgue integration on the standard
simplex. -/
theorem standard_simplex_lebesgue_integration (_k : ℕ) : True := sorry

/-- **Round 2.** *Sub-leaf of `integrationPairing`.* Linearity of
`σ ↦ ∫_σ ω` in the chain (sum-of-integrals). -/
theorem integration_linear_in_chain (_k : ℕ) : True := sorry

/-- **Round 2.** *Sub-leaf:* Linearity of `ω ↦ ∫_σ ω` in the form. -/
theorem integration_linear_in_form (_k : ℕ) : True := sorry
...
complexification). -/
theorem deRham_theorem_dimension_corollary (_k : ℕ) : True := sorry

/-- **Round 10.** *Sub-leaf of `deRhamHC1_finrank_eq_singularH1_rank`.*
Specialise the `n`-form theorem at `n = 1`. -/
theorem deRham_at_degree_one_specialisation : True := sorry
