...
`n`-simplices. -/
def cellularChain (_K : AbstractSimplicialComplex V) (_n : ℕ) : Type := sorry

instance (K : AbstractSimplicialComplex V) (n : ℕ) :
    AddCommGroup (cellularChain K n) := sorry
instance (K : AbstractSimplicialComplex V) (n : ℕ) :
    Module ℤ (cellularChain K n) := sorry

/-- The free ℤ-basis of `cellularChain K n` indexed by
`K.nSimplices n`. -/
noncomputable def cellularChain.basis
    (K : AbstractSimplicialComplex V) (n : ℕ) :
    Module.Basis (K.nSimplices n) ℤ (cellularChain K n) := sorry

/-- The cellular boundary `∂_n : C_n^cell → C_{n-1}^cell`: signed sum
of faces. -/
noncomputable def cellularBoundary
    (K : AbstractSimplicialComplex V) (n : ℕ) :
    cellularChain K (n + 1) →ₗ[ℤ] cellularChain K n := sorry

/-- `∂² = 0`. -/
theorem cellularBoundary_sq_zero
    (K : AbstractSimplicialComplex V) (n : ℕ) :
    (cellularBoundary K n).comp (cellularBoundary K (n + 1)) = 0 := sorry

/-- The cellular `H_n`: `ker(∂_n) / im(∂_{n+1})`. -/
def cellularH (_K : AbstractSimplicialComplex V) (_n : ℕ) : Type := sorry

instance (K : AbstractSimplicialComplex V) (n : ℕ) :
    AddCommGroup (cellularH K n) := sorry
instance (K : AbstractSimplicialComplex V) (n : ℕ) :
    Module ℤ (cellularH K n) := sorry
...
noncomputable def simplexCharSingular
    (K : AbstractSimplicialComplex V) (n : ℕ)
    (_s : K.nSimplices n) :
    SingularSimplex (AbstractSimplicialComplex.Geometric K) n := sorry
...
theorem cellularToSingular_isChainMap
    (_K : AbstractSimplicialComplex V) (_n : ℕ) : True := sorry
...
theorem polygon4g_cellularH1_freeZ (g : ℕ) :
    ∃ (V : Type) (K : AbstractSimplicialComplex V),
      Nonempty (cellularH K 1 ≃ₗ[ℤ] (Fin (2 * (g + 1)) → ℤ)) := sorry
...
1 face. -/
theorem polygon4g_cw_structure (_g : ℕ) : True := sorry

/-- **Round 7.** *Sub-leaf:* the relator `∏ᵢ [aᵢ, bᵢ]` abelianises to
zero in the cellular `∂_2`. -/
theorem polygon4g_cellular_d2_zero (_g : ℕ) : True := sorry

/-- **Round 8.** *Sub-leaf of `polygon4g_cellularH1_freeZ`.* `ker ∂_1 = C_1`
because `∂_1 = 0` (every edge is a loop). -/
theorem polygon4g_kernel_d1_eq_c1 (_g : ℕ) : True := sorry

/-- **Round 8.** *Sub-leaf:* `im ∂_2 = 0` because `∂_2 = 0`. -/
theorem polygon4g_image_d2_eq_zero (_g : ℕ) : True := sorry

/-- **Round 8.** *Sub-leaf:* the quotient `C_1 / 0 = C_1 = ℤ^{2(g+1)}`. -/
theorem polygon4g_cellular_h1_explicit (_g : ℕ) : True := sorry
