...
This is intentionally just a carrier, not a vector space: total addition of
divisor-compatible representatives still has to choose the resulting divisor
data. The module-facing object is `RiemannRochGermSpace` below, where addition
is performed in local meromorphic germs and pole cancellation is represented
soundly.
-/
...
/-! ### Sound germ-space module carrier for Riemann-Roch sections -/

/--
The module-facing Riemann-Roch carrier generated by divisor-compatible bounded
sections.

This is the first sound algebraic carrier for the Path B `L(D)` API: it lives
inside `MeromorphicGermFamily X`, whose addition and scalar multiplication are
defined in local `Filter.Germ`s. We deliberately do not put `AddCommGroup` or
`Module` instances on `RiemannRochBoundedSection X D`, because that record also
contains divisor representatives whose total additive result still requires
separate divisor data.
-/
noncomputable def RiemannRochGermSpace
    (X : Type*) [TopologicalSpace X] [ChartedSpace ℂ X]
    [IsManifold (modelWithCornersSelf ℂ ℂ) (⊤ : WithTop ℕ∞) X]
    [JacobianChallenge.Periods.StableChartAt ℂ X]
    (D : Divisor X) :
...
theorem germs_mem_RiemannRochGermSpace
    (s : RiemannRochBoundedSection X D) :
    s.toMeromorphicFunctionWithDivisors.germs ∈ RiemannRochGermSpace X D :=
  Submodule.subset_span (Set.mem_range_self s)

/-- The canonical inclusion of a bounded-section representative into the sound
germ-space module carrier. -/
noncomputable def toRiemannRochGermSpace
    (s : RiemannRochBoundedSection X D) :
    RiemannRochGermSpace X D :=
  ⟨s.toMeromorphicFunctionWithDivisors.germs, s.germs_mem_RiemannRochGermSpace⟩

@[simp] theorem toRiemannRochGermSpace_val
    (s : RiemannRochBoundedSection X D) :
    (s.toRiemannRochGermSpace : MeromorphicGermFamily X) =
      s.toMeromorphicFunctionWithDivisors.germs :=
  rfl

@[simp] theorem constantNonzero_toRiemannRochGermSpace_val
    (D : Divisor X) (hD : Divisor.Effective D) (c : ℂ) (hc : c ≠ 0) :
    ((constantNonzero (X := X) D hD c hc).toRiemannRochGermSpace :
      MeromorphicGermFamily X) =
      fun P => MeromorphicGermAt.constant (X := X) (p := P) c :=
  rfl

@[simp] theorem constantNonzero_toRiemannRochGermSpace_apply
    (D : Divisor X) (hD : Divisor.Effective D) (c : ℂ) (hc : c ≠ 0)
    (P : X) :
    ((constantNonzero (X := X) D hD c hc).toRiemannRochGermSpace :
      MeromorphicGermFamily X) P =
      MeromorphicGermAt.constant (X := X) (p := P) c :=
  rfl
...
The remaining frontier is to prove that this span carrier corresponds to the
intended divisor-bounded meromorphic sections and has the Riemann-Roch dimension
predicted by `analyticGenus ℂ X = 0`.
-/
noncomputable abbrev genusZero_pointRiemannRochGermSpace_finrank_target
