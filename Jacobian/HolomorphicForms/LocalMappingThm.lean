...
theorem kfold_fiber_of_conjugate_pow {k : ℕ} (hk : 0 < k) {φ : ℂ → ℂ} {z₀ : ℂ}
    ...
    ∃ r > 0,
      ∀ w : ℂ, w ≠ 0 → ‖w‖ < r →
      ∃ s : Finset ℂ, s.card = k ∧
        (↑s : Set ℂ) ⊆ Metric.ball z₀ r ∧
        (∀ t ∈ s, g t = w ∧ analyticOrderNatAt (fun u => g u - w) t = 1) ∧
        (∀ t, t ∈ Metric.ball z₀ r → g t = w → t ∈ s) := by
  -- Step 1: Get the local biholomorphism data
  obtain ⟨R, hR, δ, hδ, hInj, hSurj, hAn, hDeriv, hOrd⟩ := local_biholo_ball hφ hφ0 hφ'
  -- Step 2: Get radius where g = φ^k
  obtain ⟨r₁, hr₁, hg_eq⟩ := Metric.mem_nhds_iff.mp hg
  -- Step 3: Make r₁ ≤ R
  replace hr₁ : 0 < min r₁ R := lt_min hr₁ hR
  replace hg_eq : ∀ t ∈ Metric.ball z₀ (min r₁ R), g t = φ t ^ k :=
    fun t ht => hg_eq (Metric.ball_subset_ball (min_le_left _ _) ht)
  set r₁' := min r₁ R with hr₁'_def
  -- Step 4: Get preimage-near-center δ₁
  have hR2 : 0 < R / 2 := by positivity
  have hr1'_le : r₁' / 2 ≤ R / 2 := by simp [r₁']; linarith [min_le_right r₁ R]
  have hCont : ContinuousOn φ (Metric.closedBall z₀ (R / 2)) := by
    intro t ht
    exact (hAn t (Metric.closedBall_subset_ball (by linarith) ht)).continuousAt.continuousWithinAt
  have hInjClosed : Set.InjOn φ (Metric.closedBall z₀ (R / 2)) :=
    hInj.mono (Metric.closedBall_subset_ball (by linarith))
  -- Step 5: Choose a preliminary r' then get δ₁
  set r' := min (r₁' / 2) (δ ^ k) with hr'_def
  have hr'_pos : 0 < r' := lt_min (by positivity) (pow_pos hδ _)
  have hr'_le : r' ≤ R / 2 := le_trans (min_le_left _ _) hr1'_le
  obtain ⟨δ₁, hδ₁, hNear⟩ := preimage_near_center hR2 hCont hφ0 hInjClosed hr'_pos hr'_le
  set r := min r' (δ₁ ^ k) with hr_def
  have hr_pos : 0 < r := lt_min hr'_pos (pow_pos hδ₁ _)
  refine ⟨r, hr_pos, fun w hw hw_bound => ?_⟩
  -- Step 6: Set up the roots
  set roots := (Polynomial.X ^ k - Polynomial.C w : Polynomial ℂ).roots.toFinset
  have hroots_card : roots.card = k := pow_sub_const_roots_card hk hw
  have hroots_prop : ∀ ζ ∈ roots, ζ ^ k = w := fun ζ hζ => (mem_roots_toFinset_iff hk hw).mp hζ
  -- Step 7: All roots are in ball(0, δ)
  have hw_lt_dk : ‖w‖ < δ ^ k := by
    calc ‖w‖ < r := hw_bound
    _ ≤ r' := min_le_left _ _
    _ ≤ δ ^ k := min_le_right _ _
  have hw_lt_d1k : ‖w‖ < δ₁ ^ k :=
    lt_of_lt_of_le hw_bound (min_le_right _ _)
  have hroots_in_ball : ∀ ζ ∈ roots, ζ ∈ Metric.ball (0 : ℂ) δ := by
    intro ζ hζ
    simp only [Metric.mem_ball, dist_zero_right]
    exact kth_root_norm_lt_of_pow_norm_lt hk hδ hw (hroots_prop ζ hζ) hw_lt_dk
  -- Step 8: Lift roots through φ
  obtain ⟨preimages, hpre_card, hpre_sub, hpre_maps, hpre_complete⟩ :=
    lift_roots_through_biholo hInj hSurj roots hroots_in_ball
  -- Step 9: All preimages are near z₀ (in ball(z₀, r'))
  have hpre_near : ∀ t ∈ preimages, t ∈ Metric.ball z₀ r' := by
    intro t ht
    -- preimage t is in ball(z₀, R), and ‖φ(t)‖ < δ₁
  ...
  have hpre_in_r1 : ∀ t ∈ preimages, t ∈ Metric.ball z₀ r₁' := by
    intro t ht
    have := hpre_near t ht
    exact Metric.ball_subset_ball (le_trans (min_le_left _ _) (by linarith)) this
  refine ⟨preimages, hpre_card.trans hroots_card, hpre_in_r, ?_, ?_⟩
  -- Step 12: Each preimage maps to w and has order 1
  · intro t ht
    have ht_g : g t = w := by
      rw [hg_eq t (hpre_in_r1 t ht)]
      exact hroots_prop _ (hpre_maps t ht)
    have ht_order : analyticOrderNatAt (fun u => g u - w) t = 1 := by
      have h1 : analyticOrderNatAt (fun u => φ u ^ k - w) t = 1 := by
        have hderiv_ne : deriv φ t ≠ 0 := by
          exact hDeriv t (hpre_sub ht)
        exact order_of_pow_comp_biholo_eq_one hk (hAn t (hpre_sub ht))
          hderiv_ne hw (hroots_prop _ (hpre_maps t ht))
      rw [← h1]
      apply analyticOrderNatAt_congr
      have hball : Metric.ball z₀ r₁' ∈ 𝓝 t :=
        Metric.isOpen_ball.mem_nhds (hpre_in_r1 t ht)
      exact Filter.Eventually.mono hball (fun u hu => by rw [hg_eq u hu])
    exact ⟨ht_g, ht_order⟩
  -- Step 13: Completeness
  · intro t ht hgt
    have ht_in_r1 : t ∈ Metric.ball z₀ r₁' := by
      refine Metric.ball_subset_ball ?_ ht
      calc r ≤ r' := min_le_left _ _
      _ ≤ r₁' / 2 := min_le_left _ _
      _ ≤ r₁' := by linarith
    have ht_in_R : t ∈ Metric.ball z₀ R :=
      Metric.ball_subset_ball (min_le_right _ _) ht_in_r1
    have hφt_pow : φ t ^ k = w := by rw [← hg_eq t ht_in_r1]; exact hgt
    have hφt_root : φ t ∈ roots := (mem_roots_toFinset_iff hk hw).mpr hφt_pow
    exact hpre_complete t ht_in_R hφt_root
