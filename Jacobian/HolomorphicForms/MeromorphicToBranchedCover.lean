...
/--
Explicit data for the biholomorphic pullback construction of a simple pole.

The target lift is the standard simple-pole coordinate on `OnePoint ℂ` with
pole at `e P`; the source lift is its pullback along `e`. The remaining
analytic frontier is proving that these data satisfy the complex principal-part
...
structure BiholomorphOnePointSimplePolePullbackData
  ...
  /-- The pulled-back source coordinate on `X`. -/
  sourceLift : X → ℂ
  /-- The source coordinate is the target coordinate precomposed with `e`. -/
  sourceLift_eq : sourceLift = targetLift ∘ (e : X → OnePoint ℂ)
  /-- The pulled-back coordinate has a complex simple pole at `P`. -/
...
structure BiholomorphOnePointSimplePolePullbackFacts
    ...
    [JacobianChallenge.Periods.StableChartAt ℂ X]
    (P : X) (e : X ≃ₜ OnePoint ℂ) where
  /-- The explicit target coordinate has a complex simple pole at `e P`. -/
  targetPrincipalPart :
    HasComplexSimplePolePrincipalPart (onePointSimplePoleCoordinate (e P)) (e P)
...
This is the fully exposed shape of the remaining #234 analytic frontier:
four fields for the explicit target coordinate and four fields for its named
pullback along `e`.
-/
structure BiholomorphOnePointSimplePolePullbackFieldFacts
  ...
  /-- The target one-point extension is continuous. -/
  target_continuous_extension :
    Continuous (onePointExtend (onePointSimplePoleCoordinate (e P)) (e P))
  /-- The target coordinate has analytic order one at `e P`. -/
  target_orderAt_pole :
    JacobianChallenge.HolomorphicForms.mapAnalyticOrderAt
...
This is the remaining explicit-coordinate analytic frontier: prove the
field-level target coordinate facts and transport them along the biholomorphism
`e` to the named source coordinate.
-/
theorem biholomorphOnePointSimplePolePullbackFieldFacts_of_biholomorph_onePoint
  ...
  -- and transport meromorphicity, continuity, order-one, and
...
theorem biholomorphOnePointSimplePolePullbackFacts_of_biholomorph_onePoint
  ...
  exact ⟨{
    targetPrincipalPart := {
      ...
      continuous_extension := h.target_continuous_extension
...
back along the biholomorphism `e`, and prove the pulled-back principal-part
facts on `X`.
-/
...
theorem complexSimplePolePrincipalPart_of_biholomorph_onePoint
  ...
  obtain ⟨d⟩ :=
    biholomorphOnePointSimplePolePullbackData_of_biholomorph_onePoint X P e he he_symm
  exact ⟨d.sourceLift, d.sourcePrincipalPart⟩
