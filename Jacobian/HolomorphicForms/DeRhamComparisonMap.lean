...
It is now a **sorry-free wrapper** around the honest substrate provider
`zeroPeriodPayload_isExact_provider` (in `SmoothDifferentialForm.lean`): the
hypothesis `hω` says `(ω : SmoothDiffForm 1 X).2 = 0` (the comparison map reads
the period payload), so `ω` has zero period payload, hence is exact — i.e. it
is `dθ` for some global 0-form `θ`, which is exactly the primitive.

This replaces the earlier reduction to the closed-form *vanishing* leaf: a
zero-period closed 1-form is classically *exact, not zero*, so the honest
substrate content is exactness (`∈ ExactForm 0 X`), not vanishing. -/
theorem deRhamComparisonMap1_zero_period_primitiveExists_provider
  ...
  have hpayload : (ω : SmoothDiffForm 1 X).2 = 0 := hω
  exact zeroPeriodPayload_isExact_provider X (ω : SmoothDiffForm 1 X) hpayload
